create or replace PACKAGE BODY csr.audit_pkg AS
...
BEGIN
	SELECT 1 INTO v_dummy
	  FROM v$audit a
	 WHERE a.internal_audit_sid = in_audit_sid
	   AND ROWNUM = 1; -- Stop as soon as we see one
...
AS
	...
	v_audit_cap_table					TT_AUDIT_CAP_DATA_TABLE;
BEGIN
	-- include all only applies to this first cursor
	OPEN out_cur FOR 
	...
	SELECT TT_AUDIT_CAP_DATA_ROW (internal_audit_sid, internal_audit_type_id, flow_capability_id, permission_set)
	  BULK COLLECT INTO v_audit_cap_table
	  FROM tt_audit_capability;
	...
	OPEN out_surveys_cur FOR
		...
		SELECT iats.internal_audit_type_survey_id, iats.label,
		  ...
		  LEFT JOIN TABLE(v_audit_cap_table) sac ON sac.flow_capability_id = iatsg.survey_capability_id
										  AND sac.internal_audit_sid = in_audit_sid
		  LEFT JOIN TABLE(v_audit_cap_table) scac ON scac.flow_capability_id = iatsg.change_survey_capability_id
										   AND scac.internal_audit_sid = in_audit_sid
		 WHERE iats.app_sid = SYS_CONTEXT('SECURITY', 'APP');

	OPEN out_non_compliance_types FOR
		SELECT nct.non_compliance_type_id, tt.permission_set, NVL(fc.base_flow_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL) base_flow_capability_id
		  FROM non_compliance_type nct
		  LEFT JOIN non_compliance_type_flow_cap fc ON nct.non_compliance_type_id = fc.non_compliance_type_id AND fc.base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL
		  JOIN TABLE(v_audit_cap_table) tt ON NVL(fc.flow_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL) = tt.flow_capability_id
		 WHERE nct.app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND tt.internal_audit_sid = in_audit_sid
		 UNION
		SELECT nct.non_compliance_type_id, tt.permission_set, NVL(fc.base_flow_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS) base_flow_capability_id
		  FROM non_compliance_type nct
		  LEFT JOIN non_compliance_type_flow_cap fc ON nct.non_compliance_type_id = fc.non_compliance_type_id AND fc.base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS
		  JOIN TABLE(v_audit_cap_table) tt ON NVL(fc.flow_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS) = tt.flow_capability_id
		 WHERE nct.app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND tt.internal_audit_sid = in_audit_sid
		;
...
BEGIN
	FOR r IN (
		SELECT flow_capability_id, non_compliance_type_id
		 ...
		 WHERE app_sid = security.security_pkg.getapp
		   AND base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL
	)
	LOOP
...
BEGIN
	FOR r IN (
		SELECT fc.flow_capability_id, nct.non_compliance_type_id
		  FROM non_compliance_type nct
		  LEFT JOIN non_compliance_type_flow_cap fc ON nct.non_compliance_type_id = fc.non_compliance_type_id
			AND fc.base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL
...
AS
	v_flow_capability_id			non_compliance_type_flow_cap.flow_capability_id%TYPE;
BEGIN
	BEGIN
...
FUNCTION HasFlowAuditNonComplTypeTagAccess(
	in_non_compliance_type_id		IN	non_compliance_type.non_compliance_type_id%TYPE,
	in_audit_sid					IN	internal_audit.internal_audit_sid%TYPE,
	in_access						IN	security_pkg.T_PERMISSION
) RETURN BOOLEAN
...
BEGIN
	BEGIN
		SELECT flow_capability_id
		 ...
		 WHERE app_sid = security.security_pkg.getapp
		   AND non_compliance_type_id = in_non_compliance_type_id
		   AND base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS;
	EXCEPTION
		WHEN NO_DATA_FOUND THEN -- allow for type to be null
			v_flow_capability_id := NULL;
	END;
	RETURN HasCapabilityAccess(in_audit_sid, NVL(v_flow_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS), in_access);
END;
...
BEGIN
	...
	RETURN HasFlowAuditNonComplTypeTagAccess(v_non_compliance_type_id, in_audit_sid, in_access);
END;
...
BEGIN
	...
	DELETE FROM internal_audit_locked_tag
	 WHERE internal_audit_sid = in_sid_id;
...
BEGIN
	...
	csr_data_pkg.WriteAuditLogEntry(SYS_CONTEXT('SECURITY', 'ACT'), csr_data_pkg.AUDIT_TYPE_INTERNAL_AUDIT, SYS_CONTEXT('SECURITY', 'APP'), in_internal_audit_sid,
		'Deleted: {0} ({1})', v_description, in_internal_audit_sid);

	securableobject_pkg.DeleteSO(security_pkg.GetAct, in_internal_audit_sid);
END;
...
BEGIN
	...
	-- last ditch effort to set an auditor name
	IF v_auditor_name IS NULL AND v_auditor_user_sid IS NOT NULL THEN
		BEGIN
			SELECT full_name
			  INTO v_auditor_name
			  FROM csr.csr_user
			 WHERE csr_user_sid = v_auditor_user_sid;
		EXCEPTION
			WHEN NO_DATA_FOUND THEN
				NULL;
		END;
...
AS
BEGIN
	IF (NOT AuditExists(in_internal_audit_sid)) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_OBJECT_NOT_FOUND, 'The Audit Id ' || in_internal_audit_sid || ' could not be found.');
	END IF;
	...
	IF (NOT IsFlowAudit(in_internal_audit_sid) AND NOT HasWriteAccess(in_internal_audit_sid)) OR
	   (IsFlowAudit(in_internal_audit_sid) AND NOT HasCapabilityAccess(in_internal_audit_sid, csr_data_pkg.FLOW_CAP_AUDIT_DOCUMENTS, security.security_pkg.PERMISSION_WRITE)) THEN
			RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Access denied: You must have write access on documents on this Audit Id '|| in_internal_audit_sid);
	END IF;

	DELETE FROM internal_audit_file_data iafd
	 WHERE iafd.internal_audit_file_data_id = in_internal_audit_file_data_id;

	IF SQL%ROWCOUNT = 0 THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_OBJECT_NOT_FOUND, 'The File Id '|| in_internal_audit_file_data_id ||' could not be found.');
	END IF;

	DELETE FROM internal_audit_file iaf
	 WHERE iaf.internal_audit_file_data_id = in_internal_audit_file_data_id;
	...
	csr_data_pkg.WriteAuditLogEntry(SYS_CONTEXT('SECURITY', 'ACT'), csr_data_pkg.AUDIT_TYPE_DOCUMENTS, SYS_CONTEXT('SECURITY', 'APP'), in_internal_audit_sid,
			'Removed document with Id {0}', in_internal_audit_file_data_id);
END;
...
BEGIN
	...
	OPEN out_audit_cur FOR
		SELECT *
		  FROM (SELECT audits.*, rownum rn
		  		  FROM ( SELECT a.internal_audit_sid, a.region_sid, a.region_description, a.audit_dtm, a.label,
		 				 ...
		 				 ORDER BY a.audit_type_id, a.audit_dtm DESC) audits
	...
	OPEN out_documents_cur FOR
		SELECT *
		  FROM (SELECT documents.*, rownum rn
		  		  FROM (SELECT iafd.internal_audit_file_data_id, iaf.internal_audit_sid, iafd.filename, iafd.mime_type,
		 				 ...
		 				 WHERE (ia.flow_item_id IS NULL OR tdc.column_value IS NOT NULL)) documents
				 WHERE rownum <= in_skip_count + in_take_count)
		WHERE rn > in_skip_count;
END;
...
BEGIN
	SELECT internal_audit_sid
	  ...
	  JOIN internal_audit_type iat ON a.internal_audit_type_id = iat.internal_audit_type_id
	 WHERE iat.internal_audit_type_source_id = in_internal_audit_source_id
	   AND (in_my_audits_only = 0 OR a.auditor_user_sid = SYS_CONTEXT('SECURITY', 'SID') /*OR rrm.user_sid = v_user_sid*/)
	   AND (in_ia_type_group_id IS NULL OR NVL(a.internal_audit_type_group_id, 0) = in_ia_type_group_id)
	   AND (in_region_sid IS NULL OR a.region_sid = in_region_sid);
	...
	OPEN out_audits_cur FOR
		SELECT *
	  	  FROM(	SELECT audits.*, rownum rn
			 	  FROM (SELECT a.internal_audit_sid, a.region_sid, a.region_description, a.audit_dtm, a.label,
							a.auditor_user_sid, a.auditor_full_name, a.auditor_organisation, a.audit_type_id, a.audit_type_label internal_audit_type_label,
							a.internal_audit_type_id, a.flow_sid, a.flow_item_id, a.current_state_id, a.flow_state_label,
							a.survey_completed survey_completed_dtm, a.open_non_compliances, a.flow_state_is_final, a.audit_closure_type_id, a.closure_label
		  				  FROM v$audit a
		  				  JOIN TABLE(v_audits_t) t_a ON t_a.column_value = a.internal_audit_sid
		  				  LEFT JOIN TABLE(v_cap_clos_t) t ON a.internal_audit_sid = t.column_value
		 				 ORDER BY a.audit_type_id, a.audit_dtm DESC) audits
		...
		SELECT *
	  	  FROM(	SELECT documents.*, rownum rn
			 	  FROM (SELECT iafd.internal_audit_file_data_id, iaf.internal_audit_sid, iafd.filename, iafd.mime_type,
						...
						ORDER BY iafd.internal_audit_file_data_id) documents
				 WHERE rownum <= in_skip_count + in_take_count)
	 	 WHERE rn > in_skip_count;
END;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT *
		  FROM (SELECT audits.*, rownum rn
				  FROM ( SELECT a.internal_audit_sid, a.label, a.audit_type_label, a.region_description, a.audit_dtm, a.audit_type_id, a.region_sid
						   FROM v$audit a
						   JOIN internal_audit_type iat ON a.internal_audit_type_id = iat.internal_audit_type_id
					  	   JOIN TABLE(
								v_table
						  ...
						  WHERE iat.internal_audit_type_source_id = in_internal_audit_source_id
							...
							AND (in_my_audits_only = 0 OR a.auditor_user_sid = SYS_CONTEXT('SECURITY', 'SID') /*OR rrm.user_sid = v_user_sid*/)
							AND (in_audit_type_group_id IS NULL OR NVL(a.internal_audit_type_group_id, 0) = in_audit_type_group_id)
								OR LOWER(a.audit_type_label) LIKE v_search
								OR LOWER(a.region_description) LIKE v_search)
						  ORDER BY a.internal_audit_sid) audits
				 WHERE rownum <= in_skip_count + in_take_count)
		 WHERE rn > in_skip_count;
END;
		  ...
		  JOIN (
				SELECT ancX.app_sid, ancX.non_compliance_id, MAX(ancX.internal_audit_sid) internal_audit_sid,
				 ...
				 WHERE (in_start_dtm IS NULL OR ia.audit_dtm >= in_start_dtm)
				   ...
				   AND (in_flow_state_id IS NULL OR fi.current_state_id = in_flow_state_id)
				   AND (ia.flow_item_id IS NULL 
					OR (fc.flow_capability_id IS NULL AND cap_t.column_value IS NOT NULL)
					OR (fc.flow_capability_id IS NOT NULL AND c_cap_t.audit_sid IS NOT NULL)
				   )
...
AS
	v_action_documents_cur		SYS_REFCURSOR;
BEGIN
	GetDetails(in_sid_id, out_details_cur);
	GetExtraDetails(in_sid_id, out_nc_cur, out_nc_upload_cur, out_nc_tag_cur, out_documents_cur, v_action_documents_cur);
END;
...
BEGIN
	...
	OPEN out_action_documents_cur FOR
		 SELECT isslf.issue_log_file_id, isslf.filename, isslf.mime_type, cast(isslf.sha1 as varchar2(40)) sha1, 
		 		isslf.uploaded_dtm, issl.message, issnc.non_compliance_id owner_id, iss.issue_id issue_id
		   FROM csr.issue_log_file isslf
		   JOIN csr.issue_log issl ON issl.issue_log_id = isslf.issue_log_id AND issl.app_sid = isslf.app_sid
		   JOIN csr.issue iss ON  iss.issue_id = issl.issue_id AND iss.app_sid = issl.app_sid
		   JOIN csr.issue_non_compliance issnc ON issnc.issue_non_compliance_id = iss.issue_non_compliance_id 
				AND issnc.app_sid = iss.app_sid
		   JOIN csr.non_compliance nc ON nc.non_compliance_id = issnc.non_compliance_id AND nc.app_sid = issnc.app_sid
		   JOIN csr.internal_audit ia ON ia.internal_audit_sid = nc.created_in_audit_sid AND ia.app_sid = nc.app_sid
		   JOIN csr.audit_non_compliance anc ON nc.non_compliance_id = anc.non_compliance_id AND nc.app_sid = anc.app_sid
		   JOIN csr.internal_audit ia ON anc.internal_audit_sid = ia.internal_audit_sid AND ia.app_sid = anc.app_sid
		  WHERE isslf.app_sid = SYS_CONTEXT('SECURITY', 'APP')
		    AND ia.internal_audit_sid = in_sid_id
			AND csr.issue_pkg.sql_IsAccessAllowed(SYS_CONTEXT('SECURITY', 'ACT'),iss.issue_id) = 1
		  ORDER BY nc.non_compliance_id, isslf.issue_log_file_id;
END;
...
AS
BEGIN
	GetDetailsForMailMerge(in_sid_id, in_report_id, out_details_cur,out_nc_cur,
		out_nc_upload_cur, out_nc_tag_cur,out_issues_cur, out_template_cur,
		out_issues_fields_cur,out_issue_logs_cur,out_issue_log_files_cur,
		out_issue_action_log_cur);
...
BEGIN
	...
	OPEN out_nc_cur FOR
		SELECT nc.non_compliance_id, nc.label, nc.detail, nc.created_dtm, nc.created_by_user_sid, nc.non_compliance_ref,
...
RETURN security.T_SID_TABLE
AS
	v_permissible_nct_ids		security.T_SID_TABLE;
	v_cap_anc 					NUMBER;
	v_audit_sids				security.T_SID_TABLE := GetAuditsForUserAsTable;
	v_sids						security.T_SID_TABLE := security.T_SID_TABLE();
	v_temp_sids					security.T_SID_TABLE;
BEGIN
	-- Loop through each audit_sid
	FOR i IN 1..v_audit_sids.COUNT LOOP

		--Get the permissible nct ids for the current audit
		v_permissible_nct_ids := GetPermissibleNCTypeIds(v_audit_sids(i), security.security_pkg.PERMISSION_READ);
		--Check capability access for the current audit
		v_cap_anc := SQL_HasCapabilityAccess(v_audit_sids(i), csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL, security.security_pkg.PERMISSION_READ);

		--Insert for the current audit
		SELECT nc.non_compliance_id
		  ...
		  LEFT JOIN TABLE(v_permissible_nct_ids) pnct ON pnct.column_value = nct.non_compliance_type_id
		 WHERE nc.app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND anc.internal_audit_sid = v_audit_sids(i)
		   AND (ia.flow_item_id IS NULL OR (
			   (nc.non_compliance_type_id IS NULL AND v_cap_anc = 1) OR 
			   (nc.non_compliance_type_id IS NOT NULL AND pnct.column_value IS NOT NULL)
			))
		...
		v_sids := v_sids MULTISET UNION ALL v_temp_sids;
	END LOOP;
...
BEGIN
	...
	OPEN out_nc_cur FOR
		SELECT ia.internal_audit_sid, ia.internal_audit_ref, ia.label audit_label, ia.audit_dtm, iat.label internal_audit_type, iar.region_sid audit_region_sid,
			   iar.name audit_region_name, act.label audit_result, ia.audit_closure_type_id, audit_user.full_name audit_coordinator, ia.auditor_name auditor, ia.auditor_organisation audit_organisation, ia.notes audit_notes,
			   ia.ovw_validity_dtm,
...
AS
	...
	v_nc_ids						security_pkg.T_SID_IDS;
BEGIN
	IF NOT HasReadAccess(in_internal_audit_sid) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Access denied reading the audit with sid '||in_internal_audit_sid);
	END IF;
	...
	IF IsFlowAudit(in_internal_audit_sid) THEN
		...
		-- check user has read permissions on nc tags
		FOR i IN 1 .. v_nc_ids.count LOOP
			IF csr.audit_pkg.HasFlowAuditNonComplTagAccess(v_nc_ids(i), in_internal_audit_sid, 1) = FALSE THEN
				v_nc_ids(i) := NULL;
			END IF;
		END LOOP;
	ELSE
	...
	OPEN out_nc_tags_cur FOR
		SELECT anc.non_compliance_id finding_id, nct.tag_id, nct.tag_group_id, nct.tag
		  FROM audit_non_compliance anc
		  JOIN tag_group_ir_member nct
		  JOIN TABLE(v_nc_ids) nc ON nc.column_value = nct.non_compliance_id
			ON anc.non_compliance_id = nct.non_compliance_id
		 WHERE anc.internal_audit_sid = in_internal_audit_sid;
...
BEGIN
	...
	FOR aud IN (
		...
		IF aud.flow_item_id IS NOT NULL THEN
			-- if audit is linked to flow, then use flow capability
			IF HasFlowAuditNonComplTagAccess(
				in_non_compliance_id => in_non_compliance_id, 
				in_audit_sid => aud.internal_audit_sid, 
				in_access => in_access
			) THEN
...
BEGIN
	...
	IF in_non_compliance_id IS NULL THEN
		...
		INSERT INTO non_compliance (
			...
			region_sid, non_compliance_type_id, is_closed, root_cause, suggested_action, non_compliance_ref
		) VALUES (
			...
			v_region_sid, in_non_compliance_type_id, v_is_closed, in_root_cause, in_suggested_action, in_non_compliance_ref
	...
	ELSE
		...
		UPDATE non_compliance
		   SET label = TruncateString(in_label, 2048),
			   ...
			   root_cause = in_root_cause,
			   suggested_action = in_suggested_action,
			   non_compliance_ref = in_non_compliance_ref
		 WHERE non_compliance_id = in_non_compliance_id;
...
BEGIN
	...
	SaveNonCompliance_UNSEC(
		...
		in_non_compliance_ref		=> in_non_compliance_ref,
...
BEGIN
	...
	-- Set tags - this checks for permission on the non compliance so can't be done in unsec version
	BEGIN
		tag_pkg.SetNonComplianceTags(security_pkg.GetAct, v_non_compliance_id, in_tag_ids);
	EXCEPTION
		WHEN security_pkg.ACCESS_DENIED THEN
			-- user doesn't have permission on tags
			-- apply tags anyway if new non comp and copying from default
			IF in_non_compliance_id IS NULL AND in_from_non_comp_default_id IS NOT NULL THEN
				-- get tags from survey if we have a question id otherwise get them from the default non comp
				IF in_question_id IS NULL THEN
					SELECT ncdt.tag_id
					  BULK COLLECT INTO v_tag_ids
					  FROM csr.non_comp_default ncd
					  JOIN csr.non_comp_default_tag ncdt ON ncd.non_comp_default_id = ncdt.non_comp_default_id
					 WHERE ncd.non_comp_default_id = in_from_non_comp_default_id;
				ELSIF in_ia_type_survey_id IS NULL OR in_ia_type_survey_id = 0 THEN
					SELECT qq.tag_id
					  BULK COLLECT INTO v_tag_ids
					  FROM csr.internal_audit ia
					  JOIN csr.quick_survey_response qsr ON ia.survey_response_id = qsr.survey_response_id
					  JOIN csr.internal_audit_type iat ON ia.internal_audit_type_id = iat.internal_audit_type_id
					  JOIN csr.quick_survey qs ON qs.survey_sid = iat.default_survey_sid
					  JOIN csr.qs_question_option_nc_tag qq ON qsr.survey_sid = qq.survey_sid AND qsr.survey_version = qq.survey_version
					 WHERE ia.internal_audit_sid = in_internal_audit_sid
					   AND qq.question_id = in_question_id
					   AND qq.question_option_id = in_question_option_id;
				ELSE
					SELECT qq.tag_id
					  BULK COLLECT INTO v_tag_ids
					  FROM csr.internal_audit ia
					  JOIN csr.internal_audit_type_survey iats ON iats.internal_audit_type_id = ia.internal_audit_type_id
					  JOIN csr.internal_audit_survey ias ON ias.internal_audit_sid = ia.internal_audit_sid AND ias.internal_audit_type_survey_id = iats.internal_audit_type_survey_id
					  JOIN csr.quick_survey_response qsr ON ias.survey_response_id = qsr.survey_response_id					  
					  JOIN csr.qs_question_option_nc_tag qq ON qsr.survey_sid = qq.survey_sid AND qsr.survey_version = qq.survey_version
					 WHERE ia.internal_audit_sid = in_internal_audit_sid
					   AND qq.question_id = in_question_id
					   AND qq.question_option_id = in_question_option_id;
				END IF;
				tag_pkg.UNSEC_SetNonComplianceTags(security_pkg.GetAct, v_non_compliance_id, v_tag_ids);
			END IF;
	END;
	...
	BEGIN
		CheckNonComplianceTagAccess(v_non_compliance_id, security_pkg.PERMISSION_READ, 'Access denined reading tags to non-compliance with id: '||v_non_compliance_id);
		v_has_tag_access := 1;
	EXCEPTION
		WHEN security_pkg.ACCESS_DENIED THEN
			v_has_tag_access := 0;
...
AS
BEGIN
	SELECT NVL(qsr.survey_sid, iat.summary_survey_sid), ia.summary_response_id, qsr.guid
	  INTO out_survey_sid, out_response_id, out_guid
	  FROM internal_audit ia
	  JOIN internal_audit_type iat ON ia.internal_audit_type_id = iat.internal_audit_type_id
...
AS
BEGIN
	RETURN in_parent_lookup_key||in_prefix||NVL(in_lookup_key, in_name);
END;
...
BEGIN
	...
	FOR r IN (
	 ...
	 LEFT JOIN flow_state_audit_ind fsi
		...
		v_count := v_count + 1;
		GetOrCreateAggregateInd(v_flow_state_audit_ind_sid, r.ind_name, r.ind_name, 
			GetIndLookup(in_lookup_key, '_STATE_', r.lookup_key, r.ind_name),
		...
		SELECT COUNT(*)
		  INTO v_found
		  FROM TABLE(v_inds_t) t
		 WHERE t.column_value = v_ind_sid;
		IF v_found = 0 THEN		
			v_inds_t.EXTEND;
			v_inds_t(v_inds_t.COUNT) := v_ind_sid;
		END IF;

	END LOOP;
	
	IF v_count != v_inds_t.COUNT THEN
		RAISE_APPLICATION_ERROR(-20001, 'Not enough STATE indicators created ('|| v_count || ',' || v_inds_t.COUNT || ') - check lookup or name uniqueness.');
	END IF;

	FOR r IN (
		SELECT fsi.ind_sid, fs.lookup_key,
			   GetAuditFlowStateIndName(fs.flow_state_id, audit_pkg.IND_TYPE_FLOW_STATE_COUNT) ind_name
		  FROM flow_state fs
		  JOIN internal_audit_type iat
			ON fs.flow_sid = iat.flow_sid
	 LEFT JOIN flow_state_audit_ind fsi
		...
		-- update lookup key if the ind already existed
		-- but don't do this through the ind pkg funcs as it'll reset other stuff or fail on access problems.
		-- No need to update the name, that already happens when you save the workflow.
		UPDATE ind
		   SET lookup_key = GetIndLookup(in_lookup_key, '_STATE_', r.lookup_key, r.ind_name)
	...
	FOR r IN (
	 ...
	 LEFT JOIN flow_state_audit_ind fsi
		...
		v_count := v_count + 1;
		GetOrCreateAggregateInd(v_flow_state_time_ind_sid, r.ind_name, r.ind_name,
			GetIndLookup(in_lookup_key, '_STATE_TIME_', r.lookup_key, r.ind_name),
	...
	IF v_count != v_inds_t.COUNT THEN
		RAISE_APPLICATION_ERROR(-20001, 'Not enough STATE_TIME indicators created ('|| v_count || ',' || v_inds_t.COUNT || ') - check lookup or name uniqueness.');
	END IF;
		...
		SELECT region_sid, ind_sid, TRUNC(issue_dtm, 'MONTH'), COUNT(*)
		  FROM (
			SELECT /*+CARDINALITY(il, 250) CARDINALITY(trt, 40000) CARDINALITY(ia, 10000) CARDINALITY(iat, 10) CARDINALITY(nc, 50000)
			  ...
			  JOIN ind i ON (
					i.lookup_key = iat.lookup_key||'_ISS_RAISED' OR
					i.lookup_key = iat.lookup_key||'_ISS_REJECTED' OR
					i.lookup_key = iat.lookup_key||'_ISS_CLOSED' OR
					i.lookup_key = iat.lookup_key||'_ISS_CLOSED_OT' OR
					i.lookup_key LIKE iat.lookup_key||'_ISS_CLOSED_OD%'
			  )
			  JOIN ind_list_2 il ON i.ind_sid = il.ind_sid
			  JOIN temp_region_tree trt ON ia.region_sid = trt.child_region_sid
			 WHERE ia.app_sid = security_pkg.GetApp
		...
		SELECT region_sid, ind_sid, TRUNC(issue_dtm, 'MONTH'), COUNT(*)
		  FROM (
			...
			CROSS JOIN csr.non_compliance_type nct
				  JOIN ind i ON (
						i.lookup_key = iat.lookup_key||'_'||nct.lookup_key||'_ISS_RAISED' OR
						i.lookup_key = iat.lookup_key||'_'||nct.lookup_key||'_ISS_REJECTED' OR
						i.lookup_key = iat.lookup_key||'_'||nct.lookup_key||'_ISS_CLOSED' OR
						i.lookup_key = iat.lookup_key||'_'||nct.lookup_key||'_ISS_CLOSED_OT' OR
						i.lookup_key LIKE iat.lookup_key||'_'||nct.lookup_key||'_ISS_CLOSED_OD%'
					)
				  JOIN ind_list_2 il ON i.ind_sid = il.ind_sid
		  ...
		  FROM (
			SELECT /*+CARDINALITY(trt, 40000) CARDINALITY(ia, 10000) CARDINALITY(nc, 50000)
				   ...
				   trt.root_region_sid region_sid, j.ind_sid, NVL(iss.manual_completion_dtm, iss.resolved_dtm) resolved_dtm,
					CASE WHEN i_lookup_key = iat_lookup_key||'_'||nct_lookup_key||'_ISS_TOTAL_DAYS' AND j.override_issue_dtm = 0 THEN iss.raised_dtm
						 WHEN i_lookup_key = iat_lookup_key||'_'||nct_lookup_key||'_ISS_TOTAL_DAYS' AND j.override_issue_dtm = 1 THEN ia.audit_dtm
						 WHEN i_lookup_key = iat_lookup_key||'_'||nct_lookup_key||'_ISS_TOTAL_OD_DAYS' THEN iss.due_dtm END measure_from_dtm
			  FROM internal_audit ia
			  JOIN non_compliance nc ON ia.internal_audit_sid = nc.created_in_audit_sid AND ia.app_sid = nc.app_sid
			  JOIN (
				SELECT iat.internal_audit_type_id, nct.non_compliance_type_id, i.ind_sid, iat.override_issue_dtm,
...
BEGIN
	...
	IF (in_run_helper = 1) THEN
		INTERNAL_CallHelperPkg('AuditUpdated', in_internal_audit_sid);
	END IF;
END;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT ia.flow_item_id, fst.flow_state_transition_id, fst.verb, fst.to_state_id,
			   ...
			   fs.state_colour to_state_colour, fst.button_icon_path, fst.from_state_id
		  FROM internal_audit ia
		  JOIN flow_item fi ON ia.flow_item_id = fi.flow_item_id
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT figa.app_sid, figa.flow_item_generated_alert_id, figa.customer_alert_type_id, figa.from_state_label,
			...
			a.internal_audit_sid, a.audit_dtm, a.label, a.auditor_user_sid, NVL(cu.full_name, au.full_name) auditor_full_name, NVL(a.auditor_name, NVL(cu.full_name, au.full_name)) auditor_name, a.auditor_organisation, ac.name auditor_company,
...
AS
	...
	v_audit_abilities				T_AUDIT_ABILITY_TABLE;
BEGIN
	BEGIN
	...
	SELECT T_AUDIT_ABILITY(att.flow_capability_id, ac.permission_set) 
	  BULK COLLECT INTO v_audit_abilities
	  FROM audit_type_tab att
	  JOIN tt_audit_capability ac ON ac.internal_audit_sid = in_internal_audit_sid AND ac.flow_capability_id = att.flow_capability_id
	...
	OPEN out_cur FOR
	 ...
	 LEFT JOIN TABLE(v_audit_abilities) t ON t.flow_capability_id = att.flow_capability_id
	 LEFT JOIN customer_flow_capability cfc ON cfc.flow_capability_id = att.flow_capability_id
		 WHERE att.internal_audit_type_id = in_internal_audit_type_id
		   ...
		   AND (in_internal_audit_sid = 0 OR att.flow_capability_id IS NULL OR t.permission_set > 0)
		 GROUP BY p.plugin_id, p.plugin_type_id, p.cs_class, p.js_include, p.js_class, p.description,
			   p.details, p.preview_image_path, att.tab_label, att.pos, att.internal_audit_type_id,
			   p.tab_sid, p.form_path, p.saved_filter_sid, p.result_mode, p.pre_filter_sid,
			   att.flow_capability_id, t.permission_set, cfc.perm_type, cfc.description, p.form_sid
		 ORDER BY att.pos;
END;
...
BEGIN
	OPEN out_cur FOR 
		SELECT nct.non_compliance_type_id, nct.label, nct.lookup_key, nct.position, nct.colour_when_open,
			   nct.colour_when_closed, nct.can_have_actions, nct.closure_behaviour_id,
			   nct.score, nct.repeat_score, nct.root_cause_enabled, nct.suggested_action_enabled,
			   nct.match_repeats_by_carry_fwd, nct.match_repeats_by_default_ncs, nct.match_repeats_by_surveys,
			   nct.find_repeats_in_unit, nct.find_repeats_in_qty, nct.carry_fwd_repeat_type, nct.is_default_survey_finding,
			   NVL2 (fc.flow_capability_id, 1, 0) is_flow_capability_enabled
		  FROM non_compliance_type nct
		  LEFT JOIN non_compliance_type_flow_cap fc ON nct.non_compliance_type_id = fc.non_compliance_type_id AND fc.base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL
		 WHERE nct.app_sid = security_pkg.GetApp;
END;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT nct.non_compliance_type_id, nct.label, nct.lookup_key, nct.position, nct.colour_when_open,
			   ...
			   nct.is_default_survey_finding, fc.flow_capability_id,
			   NVL2 (fc.flow_capability_id, 1, 0) is_flow_capability_enabled
		  FROM non_compliance_type nct
		  LEFT JOIN non_compliance_type_flow_cap fc ON nct.non_compliance_type_id = fc.non_compliance_type_id AND fc.base_flow_capability_id = csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL
		  JOIN TABLE (v_type_ids) t ON nct.non_compliance_type_id = t.column_value
...
AS
	...
	v_finding_type_capability_id	non_compliance_type_flow_cap.flow_capability_id%TYPE;
...
BEGIN
	...
	IF in_non_compliance_type_id IS NULL THEN
		...
		IF v_non_compliance_type_id IS NOT NULL
		THEN
			...
			out_non_compliance_type_id := v_non_compliance_type_id;
		ELSE
			...
			VALUES (non_compliance_type_id_seq.NEXTVAL, in_label, in_lookup_key, in_position, in_colour_when_open,
					...
					v_find_repeats_in_unit, v_find_repeats_in_qty, v_carry_fwd_repeat_type, in_is_default_survey_finding)
			RETURNING non_compliance_type_id INTO out_non_compliance_type_id;
		END IF;

		BEGIN
			-- link to all existing audit types by default
			INSERT INTO non_comp_type_audit_type (internal_audit_type_id, non_compliance_type_id)
	...
	IF (in_non_compliance_type_id IS NOT NULL) THEN
		SELECT MIN(flow_capability_id)
		  INTO v_existing_flow_capability_id
		  FROM non_compliance_type_flow_cap
		 WHERE non_compliance_type_id = in_non_compliance_type_id;
	END IF;

	IF (in_is_flow_capability_enabled = 1 AND v_existing_flow_capability_id IS NULL) THEN
		csr.flow_pkg.SaveCustomerFlowCapability(
			...
			in_copy_capability_id	=> csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL,
			in_is_system_managed	=> 1,
			out_flow_capability_id	=> v_finding_type_capability_id
		);
		
		INSERT INTO csr.non_compliance_type_flow_cap (non_compliance_type_id, flow_capability_id, base_flow_capability_id)
		VALUES (out_non_compliance_type_id, v_finding_type_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NON_COMPL);
		
		csr.flow_pkg.SaveCustomerFlowCapability(
			in_flow_capability_id	=> NULL,
			in_flow_alert_class		=> 'audit',
			in_description			=> in_label || ' tags',
			in_perm_type			=> csr_data_pkg.FLOW_CAP_SPECIFIC_PERMISSION,
			in_copy_capability_id	=> csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS,
			in_is_system_managed	=> 1,
			out_flow_capability_id	=> v_finding_type_capability_id
		);
		
		INSERT INTO csr.non_compliance_type_flow_cap (non_compliance_type_id, flow_capability_id, base_flow_capability_id)
		VALUES (out_non_compliance_type_id, v_finding_type_capability_id, csr_data_pkg.FLOW_CAP_AUDIT_NC_TAGS);
	END IF;
...
BEGIN
	...
	FOR r IN (
		...
		csr.flow_pkg.DeleteCustomerFlowCapability(
			in_flow_capability_id	=> r.flow_capability_id
		);
	END LOOP;
...
BEGIN
	BEGIN
		SELECT flow_involvement_type_id
		 ...
		 WHERE UPPER(lookup_key) = UPPER(in_involv_type_lookup_key);
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
			RAISE_APPLICATION_ERROR(-20001, 'Cannot find involvement type with lookup key:'||in_involv_type_lookup_key);
	END;

	SELECT COUNT(*)
	  INTO v_count
	  FROM internal_audit ia
	  JOIN csr.flow_item_involvement fii on fii.flow_item_id = ia.flow_item_id
	 WHERE fii.flow_involvement_type_id = v_flow_involvement_type_id
	   AND ia.flow_item_id = in_flow_item_id;
...
AS
	v_response_id_table		security.T_SID_TABLE;
	v_keeper_id_tbl			security.T_SID_TABLE;
BEGIN
	IF in_keep_ia_type_survey_ids IS NULL OR (in_keep_ia_type_survey_ids.COUNT = 1 AND in_keep_ia_type_survey_ids(1) IS NULL) THEN
		-- all removed
		SELECT survey_response_id 
		  BULK COLLECT INTO v_response_id_table
		  FROM internal_audit_survey
		 WHERE survey_response_id IS NOT NULL
		   AND internal_audit_type_survey_id IN (
			SELECT DISTINCT internal_audit_type_survey_id
			  FROM internal_audit_type_survey
			 WHERE internal_audit_type_id = in_internal_audit_type_id
		 );
	ELSE
	...
	DELETE FROM csr.qs_submission_file
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);

	DELETE FROM csr.qs_response_file
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);
	...
	DELETE FROM csr.qs_answer_file
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);
	...
	UPDATE csr.internal_audit_survey
	   SET survey_response_id = NULL
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
	...
	DELETE FROM csr.quick_survey_submission
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);

	DELETE FROM csr.non_compliance_expr_action
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);
	...
	DELETE FROM csr.quick_survey_response
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);

	DELETE FROM csr.qs_response_postit
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);

	DELETE FROM csr.flow_item
	 WHERE survey_response_id IN (
		SELECT column_value
		  FROM TABLE(v_response_id_table)
		);

	DELETE FROM csr.internal_audit_survey
	...
	DELETE FROM csr.internal_audit_type_survey
	 WHERE internal_audit_type_id = in_internal_audit_type_id
	   AND internal_audit_type_survey_id NOT IN (
		SELECT column_value FROM TABLE(v_keeper_id_tbl)
	);
...
BEGIN
	...
	GetDocuments(
		in_internal_audit_sid 	=> in_sid_id,
		out_cur					=> out_audit_docs_cur
	);

	OPEN out_finding_docs_cur FOR
		SELECT nc.non_compliance_id, ncf.non_compliance_file_id, ncf.filename, ncf.mime_type, ncf.uploaded_dtm, ncf.data
		  FROM csr.non_compliance_file ncf
		  JOIN csr.non_compliance nc ON nc.non_compliance_id = ncf.non_compliance_id AND nc.app_sid = ncf.app_sid
		  JOIN csr.audit_non_compliance anc ON nc.non_compliance_id = anc.non_compliance_id AND nc.app_sid = anc.app_sid
		  JOIN csr.internal_audit ia ON anc.internal_audit_sid = ia.internal_audit_sid AND ia.app_sid = anc.app_sid
	...
	OPEN out_action_docs_cur FOR
		 SELECT isslf.issue_log_file_id, isslf.filename, isslf.mime_type,
		 		isslf.uploaded_dtm, isslf.data, iss.issue_id issue_id
		   FROM csr.issue_log_file isslf
		   JOIN csr.issue_log issl ON issl.issue_log_id = isslf.issue_log_id AND issl.app_sid = isslf.app_sid
		   JOIN csr.issue iss ON  iss.issue_id = issl.issue_id AND iss.app_sid = issl.app_sid
		   JOIN csr.issue_non_compliance issnc ON issnc.issue_non_compliance_id = iss.issue_non_compliance_id 
