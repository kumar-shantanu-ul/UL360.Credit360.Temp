CREATE OR REPLACE PACKAGE BODY cms.test_cms_role_col_pkg AS
	...
	FOR r IN (
		SELECT flow_sid
		 ...
		 WHERE flow_sid IS NOT NULL
		   AND oracle_table IN ('ROLE_TEST_1')
	) LOOP
...
BEGIN
	...
	VALUES(csr.flow_pkg.GetStateId(out_flow_sid, 'DRAFT'), cms.tab_pkg.GetColumnSid(in_tab_sid, 'ROLE_SID_1'), 1);
	
	INSERT INTO csr.flow_state_cms_col (flow_state_id, column_sid, is_editable)
	VALUES(csr.flow_pkg.GetStateId(out_flow_sid, 'CLOSED'), cms.tab_pkg.GetColumnSid(in_tab_sid, 'ROLE_SID_1'), 0);
	
	INSERT INTO csr.flow_state_transition_cms_col (flow_state_transition_id, from_state_id, column_sid)
	SELECT flow_state_transition_id, from_state_id, cms.tab_pkg.GetColumnSid(in_tab_sid, 'ROLE_SID_1')
...
BEGIN
	EXECUTE IMMEDIATE 'CREATE TABLE RAG.ROLE_TEST_1 (id number(10), role_sid_1 number(10), flow_item_id number(10), region_sid number(10) not null, constraint pk_ROLE_TEST_1 primary key (id))';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN RAG.ROLE_TEST_1.ID IS ''auto''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN RAG.ROLE_TEST_1.role_sid_1 IS ''role''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN RAG.ROLE_TEST_1.FLOW_ITEM_ID IS ''flow_item''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN RAG.ROLE_TEST_1.REGION_SID IS ''flow_region''';
	cms.tab_pkg.RegisterTable('RAG', 'ROLE_TEST_1', FALSE, FALSE);
	
	v_tab_sid := cms.tab_pkg.GetTableSid('RAG', 'ROLE_TEST_1');
	CreateWorkflow(v_tab_sid, v_flow_sid);
	
	SELECT v_region_1_1_sid
	  BULK COLLECT INTO v_region_sids
	  FROM DUAL;
	
	security_pkg.SetContext('SID', v_user_1_sid);
	csr.flow_pkg.AddCmsItem(v_flow_sid, v_region_1_1_sid, v_flow_item_id);
	EXECUTE IMMEDIATE 'INSERT INTO RAG.ROLE_TEST_1 (id, role_sid_1, region_sid, flow_item_id) VALUES (CMS.ITEM_ID_SEQ.NEXTVAL, :1, :2, :3)' USING v_role_1_sid, v_region_1_1_sid, v_flow_item_id;
	cms.tab_pkg.CheckFlowEntry(v_tab_sid, v_flow_item_id, v_region_1_1_sid);
	...
	csr.unit_test_pkg.AssertAreEqual(2, v_permission, 'User did not have write access to row they have a role on');
END;
...
AS
	...
	v_flow_state_id		NUMBER(10);
BEGIN
	csr.unit_test_pkg.StartTest('csr.test_cms_role_col_pkg.CmsRoleColNoAccess'); -- More meaningful name
	...
	EXECUTE IMMEDIATE 'INSERT INTO RAG.ROLE_TEST_1 (id, role_sid_1, region_sid, flow_item_id) VALUES (CMS.ITEM_ID_SEQ.NEXTVAL, :1, :2, :3)' USING v_role_1_sid, v_region_1_sid, v_flow_item_id;
	BEGIN
...
BEGIN
	...
	v_region_1_sid := csr.unit_test_pkg.GetOrCreateRegion('CMS_ROLE_COL_REGION_1');
	...
	v_region_1_1_sid := csr.unit_test_pkg.GetOrCreateRegion('CMS_ROLE_COL_REGION_1_1', v_region_1_sid);
	v_region_1_1_1_sid := csr.unit_test_pkg.GetOrCreateRegion('CMS_ROLE_COL_REGION_1_1_1', v_region_1_1_sid);
