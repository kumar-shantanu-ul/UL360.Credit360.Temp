...
CREATE OR REPLACE PACKAGE BODY chain.CHAIN.action_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.chain.chain_link_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.CHAIN.invitation_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.CHAIN.questionnaire_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.CHAIN.task_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.CHAIN.uninvited_pkg
IS


/***********************************************************
		PRIVATE GenerateSOName
***********************************************************/
-- CASEY: Why not move this as a public function in chain_pkg (for company_pkg as well)
FUNCTION GenerateSOName (
...
BEGIN
	...
	--      searching for a supplier and daving the component in the wizard.
	SELECT COUNT(*)
	  INTO v_count
...
BEGIN
	...
	-- CASEY: I think as people can only see their own US's, you can skip the sec check for now - just make sure that you comment why there's no sec check req'd
	IF NOT capability_pkg.CheckCapability(chain_pkg.SEND_QUESTIONNAIRE_INVITE) THEN
	...
	OPEN out_cur FOR
		SELECT sub.* FROM (
			SELECT ui.uninvited_supplier_sid as company_sid, app_sid, ctry.name as country_name, ui.name,
				   ui.country_code, row_number() OVER (ORDER BY LOWER(ui.name)) rn			  FROM uninvited_supplier ui
...
BEGIN
	SecurableObject_Pkg.CreateSO(security_pkg.GetAct, v_container_sid, class_pkg.getClassID('Chain Uninvited Supplier'), GenerateSOName(in_name, in_country_code), out_uninvited_supplier_sid);
...
END uninvited_pkg;
/
CREATE OR REPLACE PACKAGE BODY chain.chain.upload_pkg
IS
...
BEGIN
	...
	SecurableObject_pkg.CreateSO(in_act_id, in_parent_sid, class_pkg.GetClassID('ChainFileUpload'), NULL, out_file_upload_sid);

	INSERT INTO chain.file_upload
	(file_upload_sid, filename, mime_type, data, sha1) 
	SELECT out_file_upload_sid, filename, mime_type, object, 
...
AS
	v_company_sid			security_pkg.T_SID_ID DEFAULT GetCompanySid(in_file_upload_sid);
BEGIN
	...
	OPEN out_cur FOR
		SELECT file_upload_sid, filename, mime_type, last_modified_dtm, data object, NULL description,
			   NULL charset, last_modified_dtm creation_dtm, last_modified_dtm last_accessed_dtm
		  FROM chain.file_upload
		 WHERE app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND file_upload_sid = in_file_upload_sid
		   AND company_sid = v_company_sid; -- this ensures that if we bypass the capability check then we definately don't return anything
END;
