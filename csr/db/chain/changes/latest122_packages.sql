...
PROCEDURE CreateFilter (
	...
	in_field				IN  company_filter.field%TYPE,
	in_str_value			IN  company_filter.str_value%TYPE,
...
BEGIN
	...
	SELECT security.T_ORDERED_SID_ROW(company_sid, NULL)
	 ...
	 WHERE country_code IN (
	            SELECT country FROM postcode.country WHERE lower(name) like '%'||lower(in_str_value)||'%'
	        );
END;
...
BEGIN
	...
	SELECT security.T_ORDERED_SID_ROW(company_sid, NULL)
	  ...
	  FROM v$company c
	  JOIN TABLE(in_sids) t ON c.company_sid = t.sid_id
	 WHERE lower(name) like '%'||lower(in_str_value)||'%';
END;
...
AS
	v_field			company_filter.field%TYPE;
	v_str_value		company_filter.str_value%TYPE;
BEGIN
	SELECT field, str_value
	  INTO v_field, v_str_value
	  FROM company_filter
	 WHERE app_sid = security_pkg.GetApp
	   AND filter_id = in_filter_id;
	
	IF v_field = 'country' THEN
		FilterCountry(v_str_value, in_sids, out_sids);
	ELSIF v_field = 'name' THEN
		FilterName(v_str_value, in_sids, out_sids);
	END IF;
END;
...
AS
BEGIN
	out_filter_id := filter_pkg.GetNextFilterId(in_compound_filter_sid, filter_pkg.GetFilterTypeId('Chain.Cards.Filters.CompanyCore'));
	
	-- TODO: set app_sid default on table
	INSERT INTO company_filter (filter_id, field, str_value)
	VALUES (out_filter_id, in_field, in_str_value);
...
BEGIN
	...
	NULL;
	-- TODO: Add delete including helper_pkg delete
END;
...
BEGIN
	SecurableObject_pkg.CreateSO(
		security_pkg.GetAct, 
		securableobject_pkg.GetSidFromPath(security_pkg.GetAct, company_pkg.GetCompany, chain_pkg.COMPANY_UPLOADS), 
...
BEGIN
	...
	IF NOT security_pkg.IsAccessAllowedSID(security_pkg.GetAct, v_compound_filter_sid, security_pkg.PERMISSION_WRITE) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Write access denied on filter with sid: '||v_compound_filter_sid);
	END IF;
