...
CREATE OR REPLACE PACKAGE chain.cmpnt_cmpnt_relationship_pkg
IS
...
CREATE OR REPLACE PACKAGE chain.cmpnt_prod_relationship_pkg
IS
...
-- RA SPECIFC COMPONENT TYPES -- [should these go here. They are client specific - but putting theme here gives greater visibility?] => [CG: yes, they should]
...
CMPNT_TYPE_WOOD_ESTIMATE_RA		CONSTANT NUMBER := 51;
...
PROCEDURE SearchComponents ( 
	in_page   				IN  number,
	in_page_size    		IN  number,
	in_container_type_id	IN  component.component_type_id%TYPE,
	in_search_term  		IN  varchar2,
	out_count_cur			OUT	security_pkg.T_OUTPUT_CUR,
	out_result_cur			OUT	security_pkg.T_OUTPUT_CUR	
);
...
CREATE OR REPLACE PACKAGE chain.event_pkg
IS
...
PROCEDURE CreateEventType (
	...
	in_class						IN	event_type.class%TYPE,
	in_clear_urls					IN  BOOLEAN,
	in_css_class					IN	event_type.css_class%TYPE DEFAULT NULL
);
...
CREATE OR REPLACE PACKAGE chain.logical_component_pkg
IS
/*
...
END chain_link_pkg;
/
CREATE OR REPLACE PACKAGE BODY chain.cmpnt_cmpnt_relationship_pkg
IS
...
CREATE OR REPLACE PACKAGE BODY chain.cmpnt_prod_relationship_pkg
IS
...
AS
	out_count_cur				security_pkg.T_OUTPUT_CUR;
BEGIN
	GetCompProdPairs(
	in_search,				
	in_purchaser_company_sid,
	in_supplier_company_sid,	
	in_show_need_attention,	
	in_start,				
	in_page_size,			
	in_sort_by,				
	in_sort_dir,				
...
BEGIN
	...
	SELECT pc.component_id, pc.description || NVL2(component_code, ' ('||component_code||')', null) component_description, NVL(sp.product_id, DECODE(rejected, 1, 0, -1)), sp.description product_description, 
...
/**********************************************************************************
	PRIVATE
**********************************************************************************/
PROCEDURE FillComponentTypeContainment
AS
BEGIN
...
BEGIN
	...
	UPDATE component_type_containment
	 ...
	 WHERE app_sid = SYS_CONTEXT('SECURITY', 'APP')
	   AND container_component_type_id = in_container_component_type_id
	   AND child_component_type_id = in_child_component_type_id;
...
AS
	...
	v_company_sid 			component.company_sid%TYPE := NVL(component_pkg.GetComponentCompanySid(in_component_id), SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'));
...
BEGIN
	...
	IF NVL(in_component_id, 0) < 1 THEN
		...
		(component_id, component_type_id, description, component_code, created_by_sid)
		VALUES 
		(component_id_seq.nextval, in_component_type_id, in_description, in_component_code, v_user_sid) 
...
BEGIN
	v_component_ids := GetComponentTreeIds(in_top_component_id);
	...
	SELECT T_NUMERIC_ROW(component_id, rn)
	  ...
	  FROM (
	  		SELECT component_id, rownum rn 
	  		  FROM (
				SELECT in_top_component_id component_id FROM DUAL
				 UNION ALL
				SELECT component_id
				  FROM (
					SELECT component_id
					  FROM cmpnt_cmpnt_relationship
					 WHERE app_sid = security_pkg.GetApp
					 START WITH parent_component_id = in_top_component_id
...
BEGIN
	OPEN out_cur FOR
		SELECT in_top_component_id component_id, NULL parent_component_id FROM DUAL
		 UNION ALL
...
BEGIN
	...
	IF NOT IsComponentType(in_component_id, CMPNT_TYPE_NOTSURE) THEN
		RAISE_APPLICATION_ERROR(-20001, 'Cannot change component type to '||in_component_type_id||'of component with id '||in_component_id||' because it is not a NOT SURE component');
	END IF;
...
CREATE OR REPLACE PACKAGE BODY chain.event_pkg
IS
...
BEGIN
	...
	BEGIN
		INSERT INTO event_type 
		(app_sid, event_type_id, message_template, priority, class, css_class)
		VALUES
        (security_pkg.getApp, in_event_type_id, in_message_template, in_priority, in_class, in_css_class);
	EXCEPTION
	WHEN DUP_VAL_ON_INDEX THEN
		UPDATE event_type
		   SET message_template = in_message_template,
...
CREATE OR REPLACE PACKAGE BODY chain.logical_component_pkg
IS
/*
PROCEDURE AddComponent ( 
	in_description			IN  component.description%TYPE,
...
BEGIN
	...
	v_SQL := v_SQL || 		'						 purchaser_count, NVL(purchaser_company_sid, -1) purchaser_company_sid, purchaser_company_name, ''Open'' status, ';
	v_SQL := v_SQL || 		'						 DECODE(supplier_count, 0, ''-'', 1, supplier_company_name, supplier_count)  source, ';
	v_SQL := v_SQL || 		'						 DECODE(purchaser_count, 0, ''-'', 1, purchaser_company_name, purchaser_count)  purchasers ';
...
AS
	v_component_ids			T_NUMERIC_TABLE;
BEGIN
	v_component_ids := component_pkg.GetComponentTreeIds(in_top_component_id);
...
BEGIN
	...
	FOR r IN (
		...
		action_pkg.ShareQuestionnaireDoActions(in_qnr_owner_company_sid, r.share_with_company_sid, in_qt_class); 
	END LOOP;
END;
