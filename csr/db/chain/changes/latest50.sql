...
/***********************************************
Please note that this is just a terminology fixing script.
After you run these alter these few tables, it's really just a @create_views, @build
***********************************************/

-- fix up colomn names

ALTER TABLE SUPPLIER_RELATIONSHIP RENAME COLUMN OWNER_COMPANY_SID TO PURCHASER_COMPANY_SID;
ALTER TABLE ALL_PRODUCT RENAME TO PRODUCT;
ALTER TABLE ALL_COMPONENT RENAME TO COMPONENT;

-- add a missing constraint
ALTER TABLE TASK_USER_RESPONSIBLE ADD CONSTRAINT RefCHAIN_USER526 
    FOREIGN KEY (APP_SID, USER_SID)
    REFERENCES CHAIN_USER(APP_SID, USER_SID)
;

-- create dummy views for the views that are being renamed so that drop doesn't fail
create or replace view V$ALL_PRODUCT as select 1 val from dual;
create or replace view V$ALL_COMPONENT as select 1 val from dual;
create or replace view V$ALL_PRODUCT_EXTENDED as select 1 val from dual;
create or replace view V$PRODUCT_EXTENDED as select 1 val from dual;

-- drop views that are being renamed
DROP VIEW V$ALL_PRODUCT;
DROP VIEW V$ALL_COMPONENT;
DROP VIEW V$ALL_PRODUCT_EXTENDED;
DROP VIEW V$PRODUCT_EXTENDED;
...
CREATE OR REPLACE VIEW v$country AS
	SELECT country country_code, name FROM postcode.country
;   
...
***********************************************************************/
PROMPT >> Creating v$company_product by app_sid
...
SELECT product_id, p.app_sid, p.company_sid, c.name company_name, p.created_by_sid, cu.full_name created_by, p.created_dtm, p.description, p.active, 
        code_label1, code1, code_label2, code2, code_label3, code3, need_review, p.deleted, p.root_component_id
...
***********************************************************************/
PROMPT >> Creating v$company_component by app_sid
CREATE OR REPLACE VIEW v$company_component AS
SELECT component_id, cmp.app_sid, c.company_sid, c.name company_name, cmp.created_by_sid, cu.full_name created_by, cmp.created_dtm, cmp.description, 
...
***********************************************************************/
PROMPT >> Creating v$product by app_sid
CREATE OR REPLACE VIEW v$product AS
	SELECT product_id, app_sid, company_sid, company_name, created_by_sid, created_by, created_dtm, description, active, 
        code_label1, code1, code_label2, code2, code_label3, code3, need_review, root_component_id
...
/***********************************************************************
	v$component - a simple view of components 
***********************************************************************/
PROMPT >> Creating v$component by app_sid
CREATE OR REPLACE VIEW v$component AS
	SELECT component_id, app_sid, company_sid, company_name, created_by_sid, created_by, created_dtm, description
	  FROM v$company_component
	 WHERE app_sid = SYS_CONTEXT('SECURITY', 'APP')
	   AND deleted = 0
;
...
***********************************************************************/
PROMPT >> Creating v$product_relationship by app_sid
...
***********************************************************************/
PROMPT >> Creating v$product_rel_pending by app_sid
...
***********************************************************************/
PROMPT >> Creating v$company_product_extended by app_sid
...
@..\chain_pkg.sql
PROMPT Compiling cmpnt_cmpnt_relationship_pkg... 
@..\cmpnt_cmpnt_relationship_pkg.sql
PROMPT Compiling cmpnt_prod_relationship_pkg... 
@..\cmpnt_prod_relationship_pkg.sql
...
@..\chain_link_body.sql
PROMPT Compiling cmpnt_cmpnt_relationship_body... 
@..\cmpnt_cmpnt_relationship_body.sql
PROMPT Compiling cmpnt_prod_relationship_body... 
@..\cmpnt_prod_relationship_body.sql
