...
-- add component  for logged on user company
...
PROCEDURE AddComponent (
    in_company_sid        	IN  component.company_sid%TYPE,
    in_description         	IN  component.description%TYPE,
    in_component_code     	IN  component.component_code%TYPE,
	in_component_type_id	IN  component.component_type_id%TYPE,
	out_component_id		OUT component.component_id%TYPE
...
BEGIN
	...
	IF v_comp_helper IS NOT NULL THEN
		BEGIN
            execute immediate (
			'BEGIN ' || v_comp_helper || '.' || in_proc_call || ';END;');
		EXCEPTION
...
BEGIN
	...
	(component_type_id, card_text, progression_action, description_xml, position)
	VALUES
	(in_type_id, in_text, in_action, in_description, NVL(v_max_pos, 0) + 1);
...
BEGIN
	...
	IF NOT capability_pkg.CheckCapability(in_company_sid, chain_pkg.COMPONENTS, security_pkg.PERMISSION_WRITE)  THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Write access denied to company with sid '||in_company_sid);
	END IF;
	...
	RETURNING component_id INTO out_component_id;
...
BEGIN
	...
	IF NOT capability_pkg.CheckCapability(GetComponentCompanySid(in_component_id), chain_pkg.COMPONENTS, security_pkg.PERMISSION_WRITE)  THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Write access denied to company with sid '||GetComponentCompanySid(in_component_id));
	END IF;
...
BEGIN
	...
	SELECT company_sid INTO v_company_sid FROM component WHERE app_sid = SYS_CONTEXT('SECURITY', 'APP') AND component_id = in_component_id;
	
	IF NOT capability_pkg.CheckCapability(v_company_sid, chain_pkg.COMPANY, security_pkg.PERMISSION_READ)  THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Read access denied on component ' || in_component_id);
	END IF;
...
BEGIN
	...
	component_pkg.AddComponent(SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'), in_description, in_component_code, component_pkg.CMPNT_TYPE_PURCHASED, v_component_id);	
...
AS	
	v_current_supplier_company_sid	company.company_sid%TYPE;
BEGIN
	...
	IF ((in_supplier_sid <> v_current_supplier_company_sid) AND (in_supplier_sid > 0)) THEN
		cmpnt_prod_relationship_pkg.SupplierSetCmpntProdPendingRel(in_component_id, in_supplier_sid);
	END IF;
