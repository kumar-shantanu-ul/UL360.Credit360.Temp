...
ERR_COMPANY_DELETED CONSTANT NUMBER := -20510;
COMPANY_DELETED EXCEPTION;
PRAGMA EXCEPTION_INIT(COMPANY_DELETED, -20510);
...
AS
BEGIN
	RETURN securableobject_pkg.GetSIDFromPath(security_pkg.GetAct, helper_pkg.GetCompaniesContainer, GenerateSOName(in_company_name, in_country_code));
END;
...
BEGIN
	...
	IF (v_count=1) THEN
		RAISE_APPLICATION_ERROR(chain_pkg.ERR_COMPANY_DELETED, 'Company with company_sid: '||in_company_sid||' has been deleted.');
	END IF;
...
BEGIN
	...
	DELETE FROM tt_message_search
	 WHERE message_id IN (
	 	SELECT m.message_id
	 	  FROM message m, v$message_definition md, company c
	 	 WHERE m.app_sid = SYS_CONTEXT('SECURITY', 'APP')
	 	   AND m.message_definition_id = md.message_definition_id
...
BEGIN
	...
	SELECT COUNT(*) INTO v_customers 
	  FROM v$supplier_relationship
	 WHERE supplier_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')
	   AND deleted = 0; 
...
BEGIN
	...
	UPDATE tt_user_tasks paged
	   SET (message_template, re_questionnaire, re_user) = 
	   (
		SELECT msg.message_template, qt.name re_questionnaire, u.full_name re_user
		  FROM (
			SELECT m.re_company_sid, message_template, re_questionnaire_type_id, re_user_sid, re_component_id,
			 ...
			 WHERE m.re_company_sid IS NOT NULL
			   AND completion_type_id=chain_pkg.NO_COMPLETION
...
BEGIN
	...
	CASE
		WHEN v_model_id = chain_pkg.LANGUAGE_GROUP THEN
			-- try to get the file that they want
			BEGIN
				SELECT fu.file_upload_sid
				  INTO v_file_sid
				  FROM file_upload fu, file_group_file fgf
				 WHERE fu.app_sid = SYS_CONTEXT('SECURITY', 'APP')
				   AND fu.app_sid = fgf.app_sid
				   AND fu.file_upload_sid = fgf.file_upload_sid
				   AND fgf.file_group_id = in_group_id
				   AND fu.lang = SYS_CONTEXT('SECURITY', 'LANGUAGE');
			EXCEPTION
				WHEN NO_DATA_FOUND THEN
					-- TODO: try to get them the doc in the default language instead?
					NULL;
				WHEN TOO_MANY_ROWS THEN
...
BEGIN
	...
	(file_group_id, file_upload_sid)
	VALUES
	(in_group_id, v_file_sid);
...
BEGIN
	...
	OPEN out_groups_cur FOR
		SELECT file_group_id, title, description, download_permission_id, guid
		  FROM file_group
		 WHERE app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')
	...
	OPEN out_group_files_cur FOR
		SELECT fgf.file_group_id, fgf.file_upload_sid
