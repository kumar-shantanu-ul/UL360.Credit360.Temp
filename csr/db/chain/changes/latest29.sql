...
BEGIN
	...
	-- if we're checking a supplier company, ensure that they're actually a supplier
	 ...
	 AND v_company_sid <> SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY') 
	 AND company_pkg.IsSupplier(v_company_sid) = 0 
	THEN
		RETURN FALSE;
	END IF;
				...
				SELECT A.*, ROWNUM r 
				  FROM (
						SELECT c.*, NVL(sr.active, chain_pkg.inactive) active_supplier
						  FROM v$company c, (SELECT * FROM v$supplier_relationship WHERE owner_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')) sr, TABLE(in_all_results) T
		 	 			 WHERE c.app_sid = SYS_CONTEXT('SECURITY', 'APP')
...
BEGIN
	...
	DELETE FROM supplier_relationship
	 WHERE ((supplier_company_sid = in_sid_id) 
		OR (owner_company_sid = in_sid_id))	
	   AND app_sid = security_pkg.GetApp;	   
	...
	IF v_helper_pkg IS NOT NULL THEN
		EXECUTE IMMEDIATE 'begin '||v_helper_pkg||'.DeleteCompany(:1);end;'
				USING in_sid_id; -- company sid
	END IF;
...
BEGIN
	...
	SELECT security.T_ORDERED_SID_ROW(company_sid, NULL)
	  ...
	  FROM (
		 ...
		 WHERE c.app_sid = SYS_CONTEXT('SECURITY', 'APP') 
		   AND c.app_sid = sr.app_sid
		   AND sr.owner_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')
		   AND sr.owner_company_sid <> sr.supplier_company_sid
		   AND c.company_sid = sr.supplier_company_sid 
...
BEGIN
	...
	BEGIN
		...
		(owner_company_sid, supplier_company_sid)
		VALUES
		(in_owner_company_sid, in_supplier_company_sid);
	EXCEPTION
		WHEN DUP_VAL_ON_INDEX THEN
			NULL;
	END;
...
BEGIN
	...
	chain_link_pkg.ActivateRelationship(in_owner_company_sid, in_supplier_company_sid);
END;
...
BEGIN
	...
	-- PRODUCTS
	capability_pkg.RegisterCapability(chain_pkg.CT_COMPANIES, chain_pkg.PRODUCTS, chain_pkg.SPECIFIC_PERMISSION);
	capability_pkg.GrantCapability(chain_pkg.CT_COMPANY, chain_pkg.PRODUCTS, chain_pkg.ADMIN_GROUP, security_pkg.PERMISSION_READ + security_pkg.PERMISSION_WRITE);
	capability_pkg.GrantCapability(chain_pkg.CT_COMPANY, chain_pkg.PRODUCTS, chain_pkg.USER_GROUP, security_pkg.PERMISSION_READ + security_pkg.PERMISSION_WRITE);
	capability_pkg.GrantCapability(chain_pkg.CT_SUPPLIERS, chain_pkg.PRODUCTS, chain_pkg.ADMIN_GROUP, security_pkg.PERMISSION_READ);
	capability_pkg.GrantCapability(chain_pkg.CT_SUPPLIERS, chain_pkg.PRODUCTS, chain_pkg.USER_GROUP, security_pkg.PERMISSION_READ);	

	-- COMPONENTS
	capability_pkg.RegisterCapability(chain_pkg.CT_COMPANIES, chain_pkg.COMPONENTS, chain_pkg.SPECIFIC_PERMISSION);
	capability_pkg.GrantCapability(chain_pkg.CT_COMPANY, chain_pkg.COMPONENTS, chain_pkg.ADMIN_GROUP, security_pkg.PERMISSION_READ + security_pkg.PERMISSION_WRITE);
	capability_pkg.GrantCapability(chain_pkg.CT_COMPANY, chain_pkg.COMPONENTS, chain_pkg.USER_GROUP, security_pkg.PERMISSION_READ + security_pkg.PERMISSION_WRITE);
	capability_pkg.GrantCapability(chain_pkg.CT_SUPPLIERS, chain_pkg.COMPONENTS, chain_pkg.ADMIN_GROUP, security_pkg.PERMISSION_READ);
	capability_pkg.GrantCapability(chain_pkg.CT_SUPPLIERS, chain_pkg.COMPONENTS, chain_pkg.USER_GROUP, security_pkg.PERMISSION_READ);	

	FOR r IN (
		SELECT cu.host
		  FROM customer_options co, csr.customer cu
