...
Issue: Messages have not been marked as completed when a qnnaire shared resulting into being still displayed in the supplier's
'items requiring your attention' dashboard
*/
spool on;
SET wrap off;
SET linesize 9999;
spool incompleteMessage_supplier.log;
...
BEGIN
	delete from chain.xx_FB32580 
	 where app_sid = sys_context('security', 'app')
	   and share_status_id = chain.chain_pkg.SHARING_DATA
	   and primary_lookup_id = chain.chain_pkg.COMPLETE_QUESTIONNAIRE
	   and secondary_lookup_id = chain.chain_pkg.SUPPLIER_MSG;
	...
	FOR r IN (
		select md.message_template, r.to_company_sid, m.re_company_sid, q.questionnaire_id, qsle.entry_dtm, qt.questionnaire_type_id, mdl.primary_lookup_id, mdl.secondary_lookup_id, m.message_id, md.completion_type_id, m.created_dtm, 
		   ...
		   where md.completion_type_id <> 0
			 ...
			 and mdl.primary_lookup_id = chain.chain_pkg.COMPLETE_QUESTIONNAIRE--chain.chain_pkg.COMPLETE_QUESTIONNAIRE 
			 and mdl.secondary_lookup_id = chain.chain_pkg.SUPPLIER_MSG 
			 and m.re_company_sid = qs.share_with_company_sid
			 and qsle.entry_dtm > date '2013-01-01'
			 and qsle.share_status_id = chain.chain_pkg.SHARING_DATA --chain.chain_pkg.SHARING_DATA 
			 and r.to_company_sid = qs.qnr_owner_company_sid
