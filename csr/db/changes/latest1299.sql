	...
	LOOP
		BEGIN
			...
			DECLARE
				...
				v_site_regionsets_sid		security.security_pkg.T_SID_ID;
			BEGIN
				BEGIN
					v_csr_site_exists := 0;
					v_csr_site_sid := security.securableobject_pkg.GetSidFromPath(v_act_id, v_app_sid, 'wwwroot/csr/site');
					v_csr_site_exists := 1;
					v_site_regionsets_sid := security.securableobject_pkg.GetSIDFromPath(v_act_id, v_csr_site_sid, 'regionSets');
				EXCEPTION 
					WHEN security.security_pkg.OBJECT_NOT_FOUND THEN
						IF v_csr_site_exists = 1 THEN
							security.web_pkg.CreateResource(v_act_id, v_www_sid, v_csr_site_sid, 'regionSets', v_site_regionsets_sid);
							...
							security.acl_pkg.AddACE(
								v_act_id,
								security.Acl_pkg.GetDACLIDForSID(v_site_regionsets_sid),
