...
CREATE OR REPLACE VIEW csr.v$issue AS
	SELECT i.app_sid, i.issue_id, i.label, i.description, i.source_label, i.is_visible, i.source_url, i.region_sid, re.description region_name, i.parent_id,
	  ...
	  FROM issue i, issue_type ist, csr_user curai, csr_user cures, csr_user cuclo, csr_user curej, csr_user cuass, role r, correspondent c, issue_priority ip,
	       (SELECT * FROM region_role_member WHERE user_sid = SYS_CONTEXT('SECURITY', 'SID')) rrm, v$region re, issue_log lil, issue_type_rag_status itrs
...
CREATE TABLE csrimp.ISSUE_TYPE_RAG_STATUS (
    ...
    CONSTRAINT PK_ISSUE_TYPE_RAG_STATUS  PRIMARY KEY (CSRIMP_SESSION_ID, RAG_STATUS_ID),    
    CONSTRAINT FK_ISSUE_TYPE_RAG_STATUS_IS FOREIGN KEY
		(CSRIMP_SESSION_ID) REFERENCES CSRIMP.CSRIMP_SESSION (CSRIMP_SESSION_ID)
		ON DELETE CASCADE
);

CREATE TABLE csrimp.map_issue_type_rag_status (
	CSRIMP_SESSION_ID			NUMBER(10) DEFAULT SYS_CONTEXT('SECURITY', 'CSRIMP_SESSION_ID') NOT NULL,
	old_rag_status_id			NUMBER(10) NOT NULL,
	new_rag_status_id			NUMBER(10) NOT NULL,
	CONSTRAINT pk_map_issue_type_rag_status PRIMARY KEY (old_rag_status_id) USING INDEX,
	CONSTRAINT uk_map_issue_type_rag_status UNIQUE (new_rag_status_id) USING INDEX,
    CONSTRAINT FK_MAP_ISSUE_TYP_RAG_STATUS_IS FOREIGN KEY
    	(CSRIMP_SESSION_ID) REFERENCES CSRIMP.CSRIMP_SESSION (CSRIMP_SESSION_ID)
    	ON DELETE CASCADE
);

ALTER TABLE csrimp.issue ADD RAG_STATUS_ID NUMBER(10);
ALTER TABLE csrimp.issue ADD LAST_RAG_STATUS_ID NUMBER(10); 
ALTER TABLE csrimp.issue ADD VAR_EXPL CLOB;
ALTER TABLE csrimp.issue_type ADD REQUIRE_VAR_EXPL NUMBER(1,0) DEFAULT 0 NOT NULL;

grant insert on csr.issue_type_rag_status to csrimp;
grant select on csr.issue_type_rag_status_id_seq to csrimp;
