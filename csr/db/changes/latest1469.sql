...
CREATE OR REPLACE VIEW csr.v$my_section AS
    SELECT s.section_sid, firm.current_state_id, MAX(firm.is_editable) is_editable, 'F' source
     ...
     WHERE NOT EXISTS (
        ...
        SELECT null FROM csr.route WHERE route.section_sid = s.section_sid AND route.flow_state_id = firm.current_state_id
     )
...
ALTER TABLE CSR.SECTION_ROUTED_FLOW_STATE ADD (
    REJECT_FS_TRANSITION_ID        NUMBER(10)
); 
