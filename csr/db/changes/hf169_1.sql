...
BEGIN
	SELECT COUNT(*) 
	 ...
	 WHERE owner = 'CSR'
	   ...
	   AND column_name='METER_DATA_ID_OLD';

	IF v_cnt <> 0 THEN
		EXECUTE IMMEDIATE 'ALTER TABLE CSR.METER_LIVE_DATA DROP COLUMN METER_DATA_ID_OLD';
	END IF;
END;
/
...
-- No longer need the temp resequencer
