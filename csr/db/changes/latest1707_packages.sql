...
/* Capability type defines if the capability will appear undes Company, Suppliers node (a company can perform an action for the company/supplier) */
...
SEND_INVITE_ON_BEHALF_OF			CONSTANT T_CAPABILITY := 'Send invitation on behalf of';
...
--RELATIONSHIPS						CONSTANT T_CAPABILITY := 'Relationships between A and B';
...
/* 	IF helper_pkg.UseTypeCapabilities THEN
		type_capability_pkg.RegisterCapability(in_capability_type, in_capability, in_perm_type, in_is_supplier);
		RETURN;
	END IF; */
	...
	IF in_capability_type = chain_pkg.CT_COMPANIES THEN
		...
		RegisterCapability(chain_pkg.CT_COMPANY, in_capability, in_perm_type, chain_pkg.IS_NOT_SUPPLIER_CAPABILITY);
		RegisterCapability(chain_pkg.CT_SUPPLIERS, in_capability, in_perm_type, chain_pkg.IS_SUPPLIER_CAPABILITY);
		RETURN;	
	END IF;
