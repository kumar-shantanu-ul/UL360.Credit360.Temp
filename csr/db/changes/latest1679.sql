...
--Add from_company_sid to chain.v$chain_user_invitation_status
CREATE OR REPLACE VIEW chain.v$chain_user_invitation_status AS
	SELECT usr.app_sid, usr.user_sid, usr.email, usr.user_name, usr.full_name, usr.friendly_name, usr.phone_number, usr.job_title, usr.visibility_id, usr.registration_status_id, 
		usr.next_scheduled_alert_dtm, usr.receive_scheduled_alerts, usr.details_confirmed, usr.company_sid, usr.invitation_id, usr.invitation_sent_dtm, usr.invitation_status_id, usr.from_user_sid, usr.from_company_sid
	  FROM (
		SELECT vcu.app_sid, vcu.user_sid, vcu.email, vcu.user_name, vcu.full_name, vcu.friendly_name, vcu.phone_number, vcu.job_title, vcu.visibility_id, vcu.registration_status_id, 
			vcu.next_scheduled_alert_dtm, vcu.receive_scheduled_alerts, vcu.details_confirmed, i.to_company_sid company_sid, i.invitation_id, i.sent_dtm invitation_sent_dtm, i.from_user_sid, i.from_company_sid,
