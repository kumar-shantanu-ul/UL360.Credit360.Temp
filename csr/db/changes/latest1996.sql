...
CREATE TABLE CSR.ISSUE_TYPE_RAG_STATUS (
    ...
    CONSTRAINT PK_ISSUE_TYPE_RAG_STATUS  PRIMARY KEY (APP_SID, RAG_STATUS_ID),    
    CONSTRAINT FK_ISS_TYP_RAG_ST_ISS_TYP FOREIGN KEY (APP_SID, ISSUE_TYPE_ID) REFERENCES CSR.ISSUE_TYPE(APP_SID, ISSUE_TYPE_ID) ON DELETE CASCADE
);

CREATE SEQUENCE CSR.ISSUE_TYPE_RAG_STATUS_ID_SEQ
    START WITH 1
    INCREMENT BY 1
    NOMINVALUE
    NOMAXVALUE
...
CREATE INDEX CSR.UK_ISSUE_TYPE_RAG_STATUS_LABEL ON CSR.ISSUE_TYPE_RAG_STATUS(APP_SID, ISSUE_TYPE_ID, UPPER(LABEL));
CREATE INDEX CSR.UK_ISSUE_TYPE_RAG_STATUS_LKP ON CSR.ISSUE_TYPE_RAG_STATUS(APP_SID, ISSUE_TYPE_ID, UPPER(LOOKUP_KEY));

ALTER TABLE CSR.ISSUE ADD (
    RAG_STATUS_ID        NUMBER(10),
    LAST_RAG_STATUS_ID   NUMBER(10), 
	CONSTRAINT FK_ISS_ISS_TYP_RAG_STATUS FOREIGN KEY (APP_SID, RAG_STATUS_ID) REFERENCES CSR.ISSUE_TYPE_RAG_STATUS(APP_SID, RAG_STATUS_ID) ON DELETE SET NULL,
	CONSTRAINT FK_ISS_ISS_TYP_LAST_RAG_STATUS FOREIGN KEY (APP_SID, LAST_RAG_STATUS_ID) REFERENCES CSR.ISSUE_TYPE_RAG_STATUS(APP_SID, RAG_STATUS_ID) ON DELETE SET NULL
);
...
CREATE OR REPLACE VIEW csr.v$issue AS
	SELECT i.app_sid, i.issue_id, i.label, i.description, i.source_label, i.is_visible, i.source_url, i.region_sid, re.description region_name, i.parent_id,
		   ...
		   i.is_public, i.is_pending_assignment, i.rag_status_id, raised_by_user_sid, raised_dtm, curai.user_name raised_user_name, curai.full_name raised_full_name, curai.email raised_email,
...
ALTER TABLE CSR.INITIATIVE ADD DOC_LIBRARY_SID NUMBER(10, 0);

-- create doclib underneath each initiative
DECLARE
    v_doc_lib_sid 			security.security_pkg.T_SID_ID;
...
BEGIN
	...
	LOOP
		...
		FOR i IN (SELECT DISTINCT initiative_sid FROM csr.initiative)
		LOOP		
			csr.doc_lib_pkg.CreateLibrary(
				in_parent_sid_id	=> i.initiative_sid,
			...
			UPDATE csr.initiative 
			   SET doc_library_sid = v_doc_lib_sid 
			 WHERE initiative_sid = i.initiative_sid;
		END LOOP;
	END LOOP;
END;
/
