	...
	FOR r IN (
		SELECT object_owner, object_name, policy_name 
		 ...
		 WHERE pf_owner = 'CSRIMP' AND function IN ('SESSIONIDCHECK')
		   ...
		   AND object_name IN ('ISSUE_TYPE', 'ISSUE')
	) LOOP
...
ADD can_be_public NUMBER(1) DEFAULT 0 NOT NULL;

ALTER TABLE csr.issue_type
ADD CONSTRAINT CHK_CAN_BE_PUBLIC CHECK (CAN_BE_PUBLIC IN (0,1)) enable;

ALTER TABLE csr.issue
ADD is_public NUMBER(1) DEFAULT 0 NOT NULL;

ALTER TABLE csr.issue
ADD CONSTRAINT CHK_IS_PUBLIC CHECK (IS_PUBLIC IN (0,1)) enable;
 	...
 	FOR r IN (
		SELECT c.owner, c.table_name, c.nullable, (SUBSTR(c.table_name, 1, 26) || '_POL') policy_name
		 ...
		 WHERE t.owner = 'CSRIMP' AND (t.dropped = 'NO' OR t.dropped IS NULL) AND c.column_name = 'CSRIMP_SESSION_ID'
		   AND t.table_name IN ('ISSUE_TYPE', 'ISSUE')
 	)
 	LOOP
...
CREATE OR REPLACE VIEW csr.v$issue AS
	SELECT i.app_sid, i.issue_id, i.label, i.source_label, i.is_visible, i.source_url, i.region_sid, re.description region_name, i.parent_id, 
	       i.issue_escalated, i.owner_role_sid, i.owner_user_sid,
		   i.is_public, raised_by_user_sid, raised_dtm, curai.user_name raised_user_name, curai.full_name raised_full_name, curai.email raised_email,
		   ...
		   sysdate now_dtm, due_dtm, ist.issue_type_Id, ist.label issue_type_label, ist.require_priority, ist.allow_children, ist.can_be_public, i.issue_priority_id, ip.due_date_offset, CASE WHEN i.issue_priority_id IS NULL OR i.due_dtm = i.raised_dtm + ip.due_date_offset THEN 0 ELSE 1 END priority_overridden, i.first_priority_set_dtm,
