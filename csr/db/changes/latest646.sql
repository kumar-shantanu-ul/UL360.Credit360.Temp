...
CREATE OR REPLACE TYPE csr.RECURRENCE_PATTERN AS OBJECT (	
	...
	CONSTRUCTOR FUNCTION RECURRENCE_PATTERN(
		...
		recurrence_pattern_xml XMLType
...
CREATE OR REPLACE TYPE BODY csr.RECURRENCE_PATTERN AS
	...
	IS        
	BEGIN
        IF SELF.repeat_every IN ('daily','weekly') THEN
            RAISE_APPLICATION_ERROR(-20001,'Cannot set day when repeat-period is "daily" or "weekly"');
        ELSIF SELF.repeat_every IN ('monthly') AND in_day_month IS NOT NULL THEN
        ...
        ELSIF (SELF.repeat_every = 'yearly' AND in_day_month IS NULL) OR in_day_month NOT IN ('jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec') THEN
	...
	BEGIN
        ...
        ELSIF SELF.repeat_every IN ('monthly') AND in_day_varying_month IS NOT NULL THEN
        ...
        ELSIF (SELF.repeat_every = 'yearly' AND in_day_varying_month IS NULL) OR in_day_varying_month NOT IN ('jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec') THEN
