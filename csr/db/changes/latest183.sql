...
BEGIN
	...
	FOR r IN (
		select distinct i.app_sid, i.ind_sid, 0 
		  from val v, ind i
		 where trunc(period_start_dtm) <> period_start_dtm and i.ind_sid = v.ind_sid and ind_type = csr_data_pkg.ind_type_stored_calc
	)
    LOOP
	...
	where period_start_dtm <> trunc(period_start_dtm);
END;
/
