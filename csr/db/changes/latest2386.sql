...
create or replace force view csr.v$ind as
	select i.app_sid, i.ind_sid, i.parent_sid, i.name, NVL(id.description, i.name) description, i.ind_type, i.tolerance_type, 
	  ...
	  from ind i
	  left join ind_description id
	    on i.app_sid = id.app_sid AND i.ind_sid = id.ind_sid
	   AND id.lang = NVL(SYS_CONTEXT('SECURITY', 'LANGUAGE'), 'en');

create or replace view csr.v$delegation_ind as
	select di.app_sid, di.delegation_sid, di.ind_sid, di.mandatory, NVL(did.description, NVL(id.description, i.name)) description,
	  ...
	  join ind i 
	    on di.app_sid = i.app_sid and di.ind_sid = i.ind_sid;

create or replace view csr.v$region as
	select r.app_sid, r.region_sid, r.link_to_region_sid, r.parent_sid, r.name, NVL(rd.description, r.name) as description, r.active, 
	  ...
	  from region r
    LEFT JOIN csr.region_description rd ON rd.app_sid = r.app_sid 
...
create or replace view csr.v$delegation_region as
	select dr.app_sid, dr.delegation_sid, dr.region_sid, dr.mandatory, NVL(drd.description, NVL(rd.description, r.name)) description,
	  ...
	  join region r on dr.app_sid = r.app_sid AND rd.region_sid = r.region_sid;

CREATE OR REPLACE VIEW csr.v$resolved_region_description AS
	SELECT /*+ALL_ROWS*/ r.app_sid, r.region_sid, NVL(rd.description, r.name) as description, r.link_to_region_sid, r.parent_sid,
	  ...
	  LEFT JOIN region_description rd ON NVL(r.link_to_region_sid, r.region_sid) = rd.region_sid
	   AND rd.lang = NVL(SYS_CONTEXT('SECURITY', 'LANGUAGE'), 'en');
