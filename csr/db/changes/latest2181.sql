...
CREATE OR REPLACE VIEW CHAIN.v$chain_company_user AS
	...
	SELECT cu.app_sid, cu.company_sid, cu.user_sid, cu.visibility_id, NULL user_name,
	  ...
	  FROM v$company_user cu, v$company_relationship cr, company my_c
	 WHERE my_c.company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')
	   ...
	   AND (cr.company_sid IS NOT NULL OR my_c.can_see_all_companies = 1 AND my_c.company_sid != cu.company_sid)
