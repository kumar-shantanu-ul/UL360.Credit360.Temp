...
CREATE OR REPLACE VIEW chem.v$flat_substance_cas AS
	...
	FROM (	
		SELECT s.app_sid, s.substance_id, s.ref, s.description, s.classification_id, cl.description classification, 
		 ...
		 FROM substance s
			LEFT JOIN substance_cas sc ON s.substance_id = sc.substance_id
			LEFT JOIN cas c ON c.cas_code = sc.cas_code
			LEFT JOIN classification cl on cl.classification_id = s.classification_id
