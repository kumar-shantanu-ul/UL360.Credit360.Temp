...
ADD (
    IS_PENDING_ASSIGNMENT  NUMBER(1) DEFAULT 0 NOT NULL
);

ALTER TABLE csr.issue_type
ADD (
  ALLOW_PENDING_ASSIGNMENT  NUMBER(1) DEFAULT 0 NOT NULL
);
...
INSERT INTO csr.issue_action_type (issue_action_type_id, description)
VALUES (16, 'Pending assignment confirmation');
...
CREATE OR REPLACE VIEW csr.v$issue AS
	SELECT i.app_sid, i.issue_id, i.label, i.source_label, i.is_visible, i.source_url, i.region_sid, re.description region_name, i.parent_id,
		   ...
		   i.is_public, i.allow_auto_close, i.is_pending_assignment, raised_by_user_sid, raised_dtm, curai.user_name raised_user_name, curai.full_name raised_full_name, curai.email raised_email,
		   ...
		   END is_owner,
		   CASE WHEN assigned_to_user_sid = SYS_CONTEXT('SECURITY', 'SID') THEN 1 ELSE 0 --OR #### WILL WANT TO INCL. ROLES THE USER IS PART OF.
