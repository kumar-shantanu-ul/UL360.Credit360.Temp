...
PROCEDURE CheckSupplierEmailDomain (
...
CREATE OR REPLACE PACKAGE CHAIN.component_report_pkg
IS

--PROCEDURE CollectAdjustedPurchasesTest
--;

PROCEDURE CollectAdjustedPurchases (
...
PROCEDURE CopyCompoundFilter (
	in_compound_filter_sid		IN	security_pkg.T_SID_ID,
	out_new_compound_filter_sid	OUT security_pkg.T_SID_ID
);
...
PROCEDURE GetInviteDataFromId (
	in_invitation_id			IN  invitation.invitation_id%TYPE,
	out_company_sid				OUT security_pkg.T_OUTPUT_CUR
);
...
PROCEDURE ChainEnableSite (
	in_site_name				customer_options.site_name%TYPE DEFAULT NULL,
	in_initial_company_name		company.name%TYPE DEFAULT 'Credit360',
	in_initial_country_code		company.country_code%TYPE DEFAULT 'gb',
	in_link_pkg					customer_options.company_helper_sp%TYPE DEFAULT 'csr.supplier_pkg',
	in_overwrite_default_url	NUMBER DEFAULT 0,
	in_create_sectors			NUMBER DEFAULT 1,
	in_chain_implementation		customer_options.chain_implementation%TYPE DEFAULT NULL,
	in_is_single_tier			NUMBER DEFAULT 1
);

PROCEDURE ReApplyEnable (
	in_is_single_tier			NUMBER DEFAULT 1
);

-- Promoted to pkg level (from body) as it's conceivable a site might want to add sectors at a later date
...
BEGIN
	...
	dbms_output.put_line('BEGIN');
	dbms_output.put_line('    INSERT INTO chain.card_group(card_group_id, name, description)');
	...
	dbms_output.put_line('        SELECT host FROM chain.v$chain_host WHERE chain_implementation = '''||v_chain_implementation||'''');
	dbms_output.put_line('    ) LOOP');
	dbms_output.put_line('        ');
...
BEGIN
	...
	DELETE FROM newsflash_company
	 WHERE app_sid = in_app_sid;
	
	DELETE FROM product_code_type 
	 WHERE app_sid = in_app_sid;
	...
	DELETE FROM message_definition
	 WHERE app_sid = in_app_sid;
	...
	DELETE FROM alert_entry_template
	 WHERE app_sid = in_app_sid;

	DELETE FROM reason_for_action
	 WHERE app_sid = in_app_sid;
	...
	DELETE FROM company_metric_type
	 WHERE app_sid = in_app_sid;
...
PROCEDURE INTERNAL_SearchQnrsByType (
	in_phrase					IN	VARCHAR2,
	in_questionnaire_type_id	IN	questionnaire_type.questionnaire_type_id%TYPE,
	out_cur						OUT	security_pkg.T_OUTPUT_CUR
)
...
BEGIN
	OPEN out_cur FOR
		SELECT c.company_sid, c.name company_name, qt.name questionnaire_name, qs.entry_dtm submitted_dtm,
				qt.view_url, qs.share_status_name questionnaire_status_name
		  FROM v$company c
		  JOIN v$questionnaire_share qs ON c.company_sid = qs.qnr_owner_company_sid AND c.app_sid = qs.app_sid
...
AS
	v_db_class					questionnaire_type.db_class%TYPE;
BEGIN
	...
	-- On live, couldn't bind a closed cursor to EXECUTE IMMEDIATE, but worked fine locally?
	OPEN out_cur FOR
		SELECT NULL company_sid, NULL company_name, NULL questionnaire_name, NULL submitted_dtm,
				NULL view_url, NULL questionnaire_status_name
		  FROM dual
		 WHERE 1=0;
	
	SELECT MIN(qt.db_class)
	  INTO v_db_class
	  FROM questionnaire_type qt
	 WHERE qt.app_sid = SYS_CONTEXT('SECURITY', 'APP')
	   AND qt.questionnaire_type_id = in_questionnaire_type_id;
	
	IF v_db_class IS NOT NULL THEN
		BEGIN
			EXECUTE IMMEDIATE (
			'BEGIN ' || v_db_class || '.SearchQuestionnairesByType(:in_phrase, :in_questionnaire_type_id, :out_cur);END;')
			USING in_phrase, in_questionnaire_type_id, OUT out_cur;
		EXCEPTION
			WHEN PROC_NOT_FOUND THEN
				INTERNAL_SearchQnrsByType(in_phrase, in_questionnaire_type_id, out_cur);
		END;
	ELSE
		INTERNAL_SearchQnrsByType(in_phrase, in_questionnaire_type_id, out_cur);
	END IF;
...
AS
BEGIN
	RETURN ExecuteFuncReturnNumber('IsTopCompany');
END;
...
BEGIN
	...
	OPEN out_result_cur FOR 
		SELECT c.*, NVL(sr.active, chain_pkg.inactive) active_supplier, CASE WHEN sr.primary_follower_user_sid = SYS_CONTEXT('SECURITY', 'SID') THEN 1 ELSE 0 END is_primary_follower
		  FROM v$company c, TABLE(v_results) T, (
...
BEGIN
	...
	IF ((in_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')) OR (CanSeeCompanyAsChainTrnsprnt(in_company_sid))) THEN
		SELECT name
		  INTO v_n
		  FROM v$company
		 WHERE app_sid = security_pkg.GetApp
...
AS
BEGIN
	SearchSuppliers(SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'), in_page, in_page_size, in_search_term, in_only_active, out_count_cur, out_result_cur);
END;
...
BEGIN
	...
	IF (NOT capability_pkg.CheckCapability(in_company_sid, chain_pkg.SUPPLIERS, security_pkg.PERMISSION_READ)) THEN
		OPEN out_count_cur FOR
...
BEGIN
	...
	-- users can remove themselves as supplier followers, primary users can do all
	IF  in_user_sid <> SYS_CONTEXT('SECURITY', 'SID') 
	AND v_primary_user_sid <> SYS_CONTEXT('SECURITY', 'SID') 
	AND NOT capability_pkg.CheckCapability(chain_pkg.CHANGE_SUPPLIER_FOLLOWER) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Permission denied changing the primary supplier follower for the company with sid '||in_supplier_company_sid);
	END IF;
	...
	message_pkg.RemoveCompanyFollowerMessages(SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'), in_supplier_company_sid, in_user_sid);
...
BEGIN
	...
	OPEN out_result_cur FOR
		SELECT c.*, u.*
		  ...
		  JOIN TABLE(v_results) f ON f.column_value = c.company_sid;
END;
...
BEGIN
	...
	IF NOT chain.capability_pkg.CheckCapability(chain_pkg.SEND_QUESTIONNAIRE_INVITE) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Access denied checking email domains');
	END IF;
		
	OPEN out_cur FOR
		SELECT c.company_sid, c.name
		  FROM v$company c
		  JOIN v$supplier_relationship sr ON c.company_sid = sr.supplier_company_sid AND c.app_sid = sr.app_sid
		 WHERE sr.purchaser_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')
		   AND c.company_sid IN (
			SELECT DISTINCT cu.company_sid
			  FROM v$company_user cu
			 WHERE LOWER(email) LIKE '%@'||LOWER(in_domain)
			   AND LOWER(email) NOT LIKE '%@credit360.com'
			)
		   AND ROWNUM<10; -- The user can't do much with more than this
END;
...
AS
	v_phrase			filter_value.str_value%TYPE;
BEGIN
	SELECT ff.str_value
	  INTO v_phrase
	  FROM v$filter_value ff
	...
	SELECT company_sid
	  ...
	  FROM (
		SELECT DISTINCT company_sid
		  FROM v$company c
		  JOIN TABLE(in_sids) t ON c.company_sid = t.column_value
		 WHERE LOWER(c.name) LIKE '%'||LTRIM(RTRIM(v_phrase))||'%'
		    OR (c.app_sid, c.company_sid) IN (
				SELECT ssr.app_sid, ssr.supplier_sid
				  FROM csr.supplier_survey_response ssr
				  JOIN v$questionnaire_share q ON q.questionnaire_type_id = ssr.survey_sid AND q.qnr_owner_company_sid = ssr.supplier_sid
				  JOIN csr.quick_survey_answer qsa ON ssr.survey_response_id = qsa.survey_response_id AND ssr.app_sid = qsa.app_sid
				 WHERE LOWER(qsa.answer) LIKE '%'||LTRIM(RTRIM(v_phrase))||'%'
				)
		    OR (c.app_sid, c.company_sid) IN (
				SELECT ssr.app_sid, ssr.supplier_sid
				  FROM csr.supplier_survey_response ssr
				  JOIN v$questionnaire_share q ON q.questionnaire_type_id = ssr.survey_sid AND q.qnr_owner_company_sid = ssr.supplier_sid
				  JOIN csr.qs_answer_file qsaf ON ssr.survey_response_id = qsaf.survey_response_id AND ssr.app_sid = qsaf.app_sid
				 WHERE CONTAINS(qsaf.data, v_phrase, 1) > 0
				)
		    -- Can include the below when considering permissions properly, otherwise users could use
		    -- search terms to guess the contents of files they don't have read access to
		    /*OR (c.app_sid, c.company_sid) IN (
				SELECT fu.app_sid, fu.company_sid
				  FROM file_upload fu
				 WHERE CONTAINS(fu.data, v_phrase, 1) > 0
				   AND fu.download_permission_id IN (chain_pkg.DOWNLOAD_PERM_STANDARD, chain_pkg.DOWNLOAD_PERM_EVERYONE, chain_pkg.DOWNLOAD_PERM_SUPPLIERS)
				)*/
		);
END;
...
BEGIN
	...
	FOR r IN (
		...
		ELSIF r.name = 'Search' THEN
			Search(in_filter_id, r.filter_field_id, out_sids, out_sids);
		END IF;
...
BEGIN
	...
	filter_pkg.CreateCompoundFilter(23 /*Basic company filter*/, out_compound_filter_sid);
	filter_pkg.AddCardFilter(out_compound_filter_sid, 'Credit360.Chain.Cards.Filters.CompanyCore', 23, v_filter_id);
	filter_pkg.AddFilterField(v_filter_id, 'Search', 'search', v_field_id);
	filter_pkg.AddStringValue(v_field_id, in_search_phrase, v_value_id);
END;
...
BEGIN
	...
	ELSE
		SELECT column_value
		  BULK COLLECT INTO v_filtered_results
		  FROM v$chain_company_user ccu, TABLE(in_all_results) T
		 WHERE ccu.app_sid = SYS_CONTEXT('SECURITY', 'APP')
		   AND ccu.company_sid = in_company_sid
		   AND ccu.user_sid = T.column_value;
	END IF;
	...
	IF NVL(in_page_size, 0) = 0 THEN	
		 OPEN out_result_cur FOR 
			SELECT * FROM (
				SELECT cu.*, in_company_sid company_sid, CASE WHEN v_show_admins = 1 THEN NVL(ca.user_sid, 0) ELSE 0 END is_admin
...
BEGIN
	...
	--  1 Is this my company
	--  2 Is this a supplier (e.g. they have accepted an invitation from my company)
	IF  (v_supplier_relationship > 0) OR (in_company_sid = SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY')) THEN 
...
BEGIN
	...
	CollectSearchResults(SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'), v_results, FALSE, 0, NULL, out_count_cur, out_result_cur);
END;
...
BEGIN
	...
	CollectSearchResults(SYS_CONTEXT('SECURITY', 'CHAIN_COMPANY'), v_results, FALSE, NULL, NULL, v_count_cur, out_cur);
END;
...
BEGIN
	...
	-- conversion_to_base is what you multiply the metric value to 
	BEGIN
...
BEGIN
	...
	OPEN out_component_cur FOR
		SELECT component_id, description, 
			   ...
			   supplier_product_published, supplier_product_published_dtm, cp.purchases_locked
...
CREATE OR REPLACE PACKAGE BODY CHAIN.component_report_pkg
IS
...
--	  WHERE company_sid = 179797
--	  AND pc.purchases_locked = 1;
--	
--	CollectAdjustedPurchases(to_date('01/01/2011', 'dd/mm/yyyy'), to_date('01/01/2012', 'dd/mm/yyyy'), 'h', v_component_ids);
--	
--	/*
--	
--	INSERT INTO CHAIN.TT_PURCHASES (
--		component_id,        
--		company_sid,         
--		purchase_channel_id,    
--		start_date,          
...
--	)
--	SELECT 
--			pc.component_id, pc.company_sid,
--			pu.purchase_channel_id, start_date, end_date, amount raw_amount, 
--			CASE
--				-- invoice in range 
--				WHEN ((start_date >= '1 jan 2011') AND (start_date < '1 jan 2012') AND (end_date IS NULL)) THEN amount
--				-- date range fully in range 
--				WHEN ((start_date >= '1 jan 2011') AND (end_date < '1 jan 2012')) THEN amount
--				-- date range - starts in range ends out of range 
--				WHEN ((start_date >= '1 jan 2011') AND (start_date < '1 jan 2012') AND (end_date >= '1 jan 2012')) THEN (amount * (to_date('01/01/2012', 'dd/mm/yyyy') - start_date) / (end_date - start_date)) 
--				-- date range - starts out of range ends in range 
--				WHEN ((start_date < '1 jan 2011') AND (end_date < '1 jan 2012') AND (end_date >='1 jan 2011')) THEN (amount * (end_date - to_date('01/01/2011', 'dd/mm/yyyy')) / (end_date - start_date)) 
--				-- date range - fully covers range but starts in and out of range 
--				WHEN ((start_date < '1 jan 2011') AND (end_date >= '1 jan 2012')) THEN (amount * (to_date('01/01/2012', 'dd/mm/yyyy') - to_date('01/01/2011', 'dd/mm/yyyy')) / (end_date - start_date)) 
--				ELSE null
--			END amount, pu.amount_unit_id, a.unit_type, conversion_to_base, pc.supplier_product_id
--	  FROM purchase pu, v$purchased_component pc, TABLE(in_product_ids) i, v$product p, amount_unit a
--	 WHERE pc.component_id = pu.product_id AND pc.app_sid = pu.app_sid 
--	   AND pu.amount_unit_id = a.amount_unit_id AND pu.app_sid = a.app_sid  
...
AS
	v_period_start_dtm DATE := in_start_date;
...
BEGIN
	...
	-- find the end date for our new period
	...
	ELSE
		RAISE_APPLICATION_ERROR(-20001, 'Unknown interval: '||in_interval);
	END IF;
 
    v_loop_id := 1;
    WHILE v_period_start_dtm < in_end_date
    LOOP   
        v_period_end_dtm := ADD_MONTHS(v_period_start_dtm, v_interval_months);  		
		
		-- fill the table - it there a more efficient way of doing this?
		INSERT INTO CHAIN.TT_PURCHASES (
			period_id, 
			period_start_date,          
			period_end_date,
		...
		SELECT 
				v_loop_id, v_period_start_dtm, v_period_end_dtm, 
				pc.component_id, pc.company_sid,
				pu.purchase_channel_id, start_date, end_date, amount raw_amount, 
				CASE
					-- invoice in range 
					WHEN ((start_date >= v_period_start_dtm) AND (start_date < v_period_end_dtm) AND (end_date IS NULL)) THEN amount
					-- date range fully in range 
					WHEN ((start_date >= v_period_start_dtm) AND (end_date < v_period_end_dtm)) THEN amount
					-- date range - starts in range ends out of range 
					WHEN ((start_date >= v_period_start_dtm) AND (start_date < v_period_end_dtm) AND (end_date >= v_period_end_dtm)) THEN (amount * (v_period_end_dtm - start_date) / (end_date - start_date)) 
					-- date range - starts out of range ends in range 
					WHEN ((start_date < v_period_start_dtm) AND (end_date < v_period_end_dtm) AND (end_date >=v_period_start_dtm)) THEN (amount * (end_date - v_period_start_dtm) / (end_date - start_date)) 
					-- date range - fully covers range but starts in and out of range 
					WHEN ((start_date < v_period_start_dtm) AND (end_date >= v_period_end_dtm)) THEN (amount * (v_period_end_dtm - v_period_start_dtm) / (end_date - start_date)) 
					ELSE null
				END amount, pu.amount_unit_id, a.unit_type, conversion_to_base, pc.supplier_product_id
		  FROM purchase pu, v$purchased_component pc, TABLE(in_product_ids) i, v$product p, amount_unit a
		 WHERE pc.component_id = pu.product_id AND pc.app_sid = pu.app_sid 
		   ...
		   AND pc.purchases_locked = 1
		   AND pc.deleted = chain_pkg.NOT_DELETED
		   AND pc.supplier_product_id = i.column_value
		   AND pc.app_sid = SYS_CONTEXT('SECURITY','APP');
...
BEGIN
	...
	INSERT INTO compound_filter (compound_filter_sid, card_group_id)
	VALUES (out_compound_filter_sid, in_card_group_id);
END;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT cf.compound_filter_sid, cf.name
		  FROM compound_filter cf
		  JOIN TABLE(SecurableObject_pkg.GetChildrenWithPermAsTable(SYS_CONTEXT('SECURITY', 'ACT'), 
				v_parent_sid, security_pkg.PERMISSION_READ)) so ON cf.compound_filter_sid = so.sid_id
		 WHERE cf.name IS NOT NULL
...
BEGIN
	...
	SELECT card_group_id
	 ...
	 WHERE app_sid = security_pkg.GetApp
	   AND compound_filter_sid = in_compound_filter_sid;
	
	CreateCompoundFilter(v_card_group_id, out_new_compound_filter_sid);
	...
	FOR r IN (
		...
		INSERT INTO filter (filter_id, filter_type_id, compound_filter_sid, operator_type)
		VALUES (filter_id_seq.NEXTVAL, r.filter_type_id, out_new_compound_filter_sid, r.operator_type)
...
BEGIN
	...
	VALUES (filter_id_seq.NEXTVAL, in_filter_type_id, in_compound_filter_sid)
	RETURNING filter_id INTO v_filter_id;
...
AS
BEGIN
	IF NOT security_pkg.IsAccessAllowedSID(security_pkg.GetAct, GetCompoundSidFromfilterId(in_filter_id), security_pkg.PERMISSION_READ) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Read access denied on filter with sid: '||GetCompoundSidFromfilterId(in_filter_id));
	END IF;
	
	OPEN out_cur FOR
		SELECT filter_id, filter_type_id, compound_filter_sid, operator_type
		  FROM filter
		 WHERE app_sid = security_pkg.GetApp
		   AND filter_id = in_filter_id;
END;
...
BEGIN
	FOR r IN (
		...
		INSERT INTO filter_value (filter_value_id, filter_field_id, num_value, str_value, dtm_value)
		SELECT filter_value_id_seq.NEXTVAL, v_filter_field_id, num_value, str_value, dtm_value
		  FROM filter_value
		 WHERE app_sid = security_pkg.GetApp
...
BEGIN
	...
	FOR r IN (
		...
		EXECUTE IMMEDIATE ('BEGIN ' || r.helper_pkg || '.FilterCompanySids(:filter_id, :input, :output);END;') USING r.filter_id, v_list, OUT v_list;
	END LOOP;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT null name, null number_value, null string_value
          from dual where 1 = 0;
end;
...
BEGIN
	OPEN out_cur FOR
		SELECT * FROM v$country
		 WHERE country_code IN (SELECT DISTINCT country_code FROM v$company)
		 ORDER BY LOWER(name);
END;
...
BEGIN
	OPEN out_cur FOR
		SELECT sector_id, description
		 ...
		 WHERE app_sid = security_pkg.GetApp
		   AND active = chain_pkg.ACTIVE
		   AND sector_id IN (SELECT sector_id FROM v$company)
		 ORDER BY LOWER(description);
END;
...
BEGIN
	...
	OPEN out_invitation_cur FOR
		SELECT opt.site_name, tc.company_sid to_company_sid, tc.name to_company_name, tu.full_name to_user_name, fc.name from_company_name, fu.full_name from_user_name,
				tu.registration_status_id, i.guid, tu.email to_user_email, tu.user_name to_user_id
...
BEGIN
	...
	IF in_reason <> chain_pkg.REJECTED_NOT_EMPLOYEE AND in_reason <> chain_pkg.REJECTED_NOT_SUPPLIER AND in_reason <> chain_pkg.ANOTHER_USER_REGISTERED AND in_reason <> chain_pkg.CANNOT_ACCEPT_TERMS THEN
...
BEGIN
	...
	OPEN out_company_sid FOR
		SELECT to_company_sid, invitation_status_id
		  FROM invitation
		 WHERE app_sid = security_pkg.GetApp
		   AND invitation_id = in_invitation_id;	  
END;
...
AS
	v_has_full_invite_list_access	NUMBER := 0;
BEGIN
	IF NOT capability_pkg.CheckCapability(chain_pkg.SEND_QUESTIONNAIRE_INVITE) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Access denied sending questionnaire invite');
	END IF;
...
BEGIN
	...
	SELECT SYS_CONTEXT('SECURITY', 'APP'), v_invitation_id,
		   ...
		   from_user_sid, -- SYS_CONTEXT('SECURITY', 'SID'), -- should the invitation come from the original sender or the person resending?
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT pc.component_id, pc.description, pc.component_code, pc.component_notes, pc.company_sid, 
				...
				pc.supplier_product_published, pc.supplier_product_published_dtm, pc.purchases_locked
...
BEGIN
	IF NOT capability_pkg.CheckCapability(chain_pkg.IS_TOP_COMPANY) THEN
		RAISE_APPLICATION_ERROR(security_pkg.ERR_ACCESS_DENIED, 'Access denied searching for purchases for product '||in_product_id||'. Current company ('||SYS_CONTEXT('SECURITY','CHAIN_COMPANY')||') is not a top level company');
	END IF;
...
BEGIN
	...
	UPDATE questionnaire_type
	   SET active=0
	 WHERE app_sid=security_pkg.getApp
	   AND questionnaire_type_id= 12021715;
END;
...
AS
	v_filtered_sids				security.T_SID_TABLE;
...
BEGIN
	...
	OPEN out_cur FOR
		SELECT c.company_sid "ID",
		        ...
		        c.website
		  FROM v$company c
			JOIN TABLE(v_filtered_sids) f ON c.company_sid = f.column_value
...
PROCEDURE CreateCompany(
	in_initial_company_name			company.name%TYPE,
	in_initial_country_code			company.country_code%TYPE
)
AS
	v_act_id					security_pkg.T_ACT_ID DEFAULT security_pkg.GetAct;
	v_admins					security_pkg.T_SID_ID DEFAULT securableobject_pkg.GetSIDFromPath(v_act_id, security_pkg.GetApp, 'Groups/Administrators');
	v_company_sid				security_pkg.T_SID_ID;
	...
	v_chain_users_group_sid		security_pkg.T_SID_ID DEFAULT securableobject_pkg.GetSIDFromPath(v_act_id, security_pkg.GetApp, 'Groups/Chain Users');
	v_menu_sid					security_pkg.T_SID_ID;
BEGIN		
	BEGIN
		company_pkg.CreateUniqueCompany(in_initial_company_name, in_initial_country_code, NULL, v_company_sid);
	EXCEPTION
		WHEN DUP_VAL_ON_INDEX THEN
			v_company_sid := securableobject_pkg.GetSIDFromPath(v_act_id, security_pkg.GetApp, 'Chain/Companies/'||in_initial_company_name||' ('||in_initial_country_code||')');
	END;
	...
	v_menu_sid := securableobject_pkg.GetSidFromPath(v_act_id, security_pkg.GetApp, 'menu/chain/chain_supplier_search');
	...
	FOR r IN (
		...
		chain.company_user_pkg.AddUserToCompany(v_company_sid, r.csr_user_sid);
		...
		chain.company_user_pkg.ApproveUser(v_company_sid, r.csr_user_sid);
	END LOOP;

	-- I'm less interested in setting up users and address's etc - that ought to be for client folders to set up
	/*FOR r IN (
		SELECT 'casey' name FROM DUAL
		UNION
		SELECT 'james' name FROM DUAL
		UNION
		SELECT 'richard' name FROM DUAL
		UNION
		SELECT 'anthony' name FROM DUAL
	) 
	LOOP
		BEGIN
			v_user_sid := company_user_pkg.CreateUser(v_company_sid, r.name, 'redfive55', r.name, r.name||'@credit360.com'); 
		EXCEPTION
			WHEN security_pkg.DUPLICATE_OBJECT_NAME THEN
				v_user_sid := securableobject_pkg.GetSIDFromPath(v_act_id, security_pkg.GetApp, 'Users/'||r.name||'@credit360.com');
		END;
		-- ensure that they're active
		csr.csr_user_pkg.ActivateUser(security_pkg.GetAct, v_user_sid);
		-- add to site admins group
		--group_pkg.AddMember(security_pkg.GetAct, v_user_sid, v_admins);
		-- add them to The Company
		company_user_pkg.AddUserToCompany(v_company_sid, v_user_sid);
		-- approve these users
		company_user_pkg.ApproveUser(v_company_sid, v_user_sid);
		-- show all
		company_user_pkg.SetVisibility(v_user_sid, chain_pkg.FULL); -- bad name!
		
		-- we've put a hack in to set users to accepted in the main code so do that here
		UPDATE chain_user 
		    SET registration_status_id = chain_pkg.REGISTERED,
				default_company_sid = v_company_sid
		  WHERE user_sid = v_user_sid 
		    AND app_sid = security_pkg.GetApp;		    
	END LOOP;*/

	/*UPDATE company 
	   SET address_1 = 'address 1', 
	   	   address_2 = 'address 2', 
	   	   address_3 = 'address 3', 
	   	   address_4 = 'address 4', 
	   	   town = 'town',
	...
	-- allow this company only to invite suppliers
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.USER_GROUP, chain_pkg.CT_COMMON, chain_pkg.SEND_QUESTIONNAIRE_INVITE);
	-- allow all users of this company only to write to supplier details
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.USER_GROUP, chain_pkg.CT_ROOT, chain_pkg.SUPPLIERS, security_pkg.PERMISSION_READ + security_pkg.PERMISSION_WRITE);
	-- allow company admins to setup email stub registration
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.ADMIN_GROUP, chain_pkg.CT_COMPANY, chain_pkg.SETUP_STUB_REGISTRATION);
	-- allow company admins to setup survey questionnaires
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.ADMIN_GROUP, chain_pkg.CT_COMPANY, chain_pkg.CREATE_QUESTIONNAIRE_TYPE);
	-- allow company admins to send newsflashes
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.ADMIN_GROUP, chain_pkg.CT_COMMON, chain_pkg.SEND_NEWSFLASH);
	-- allow company admins and users to receive user-targeted newsflashes
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.ADMIN_GROUP, chain_pkg.CT_COMMON, chain_pkg.RECEIVE_USER_TARGETED_NEWS);
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.USER_GROUP, chain_pkg.CT_COMMON, chain_pkg.RECEIVE_USER_TARGETED_NEWS);
	...
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.ADMIN_GROUP, chain_pkg.CT_SUPPLIERS, chain_pkg.RESET_PASSWORD);
	-- If company users have permission to create users, then also let them specify a user name.
	capability_pkg.SetCapabilityPermission(v_company_sid, chain_pkg.USER_GROUP, chain_pkg.CT_COMPANY, chain_pkg.SPECIFY_USER_NAME);
...
--FROM \cvs\csr\db\utils\EnableChain\sectors
PROCEDURE CreateSectors
AS
BEGIN
...
-- FROM \cvs\csr\db\utils\EnableChain\portlets
...
BEGIN
	...
	INSERT INTO csr.customer_portlet (portlet_id, app_sid, portal_group)
	...
	MINUS SELECT portlet_id, app_sid, portal_group FROM csr.customer_portlet WHERE app_sid = v_app_sid;

	--user_pkg.LogonAuthenticated(securableobject_pkg.GetSidFromPath(v_act, v_app_sid, 'users/admin'), 600, v_act);

	csr.portlet_pkg.AddTabReturnTabId(v_app_sid, 'Dashboard', 1 /* shared */,  2 /* layout full width */, 'Chain', v_tab_id);

	select portlet_id into v_portlet_id from csr.portlet where type = 'Credit360.Portlets.Chain.RecentActivity';
	csr.portlet_pkg.AddPortletToTab(v_tab_id, v_portlet_id, '{"portletTitle":"Latest Activity"}', v_tab_portlet_id);

	select portlet_id into v_portlet_id from csr.portlet where type = 'Credit360.Portlets.Chain.InvitationSummary';
	csr.portlet_pkg.AddPortletToTab(v_tab_id, v_portlet_id, '{"portletTitle":"System Summary"}', v_tab_portlet_id);

	select portlet_id into v_portlet_id from csr.portlet where type = 'Credit360.Portlets.Chain.NewsFlash';
	csr.portlet_pkg.AddPortletToTab(v_tab_id, v_portlet_id, '{"portletTitle":"News"}', v_tab_portlet_id);

	select portlet_id into v_portlet_id from csr.portlet where type = 'Credit360.Portlets.Chain.RequiredActions';
	csr.portlet_pkg.AddPortletToTab(v_tab_id, v_portlet_id, '{"portletTitle":"Items requiring your attention"}', v_tab_portlet_id);

	v_tab_ids(1) := v_tab_id;
	csr.portlet_pkg.SetTabsForGroup('Chain', v_chain_users_sid, v_tab_ids);
end;

PROCEDURE CreateCsrAlerts
AS
	v_sat_ids					security_pkg.T_SID_IDS; -- std_alert_type ids
...
BEGIN
	...
	FOR r IN (
		...
		VALUES
			...
			'<template><div>Dear <mergefield name="FRIENDLY_NAME"/>,<br/><br/>To finish your registration process, click the link or copy and paste it into your web browser.<br/><br/><mergefield name="LINK"/><br/><br/>This link expires <mergefield name="EXPIRATION"/>.</div></template>', 
			'<template />');
		...
		VALUES
			...
			'<template><mergefield name="COMPANY_NAME"/> - <mergefield name="SITE_NAME"/> Recent Activity</template>', 
			'<template><div>Dear <mergefield name="FRIENDLY_NAME"/>,<br/><br/>Below is a list of recent activity involving you or <mergefield name="COMPANY_NAME"/> on <mergefield name="SITE_NAME"/>.<br/><br/><mergefield name="ENTRIES"/></div></template>', 
			'<template />');
...
AS
BEGIN
	message_pkg.OverrideMessageDefinition(
		in_primary_lookup 			=> chain_pkg.CONFIRM_COMPANY_DETAILS,
...
PROCEDURE CreateCapabilities
AS
BEGIN
	-- TODO: If these are becoming the "Default", then why are we overriding them here rather than updating the default?

	capability_pkg.OverrideCapability(chain_pkg.CT_COMPANY, chain_pkg.SUBMIT_QUESTIONNAIRE, chain_pkg.USER_GROUP);
	
	capability_pkg.HideCapability(chain_pkg.SUPPLIERS, chain_pkg.ADMIN_GROUP);
	capability_pkg.HideCapability(chain_pkg.SUPPLIERS, chain_pkg.USER_GROUP);
	
	capability_pkg.HideCapability(chain_pkg.SEND_QUESTIONNAIRE_INVITE, chain_pkg.ADMIN_GROUP);
END;

PROCEDURE CreateCards
AS
BEGIN
	-- TODO: This can be moved to core chain
...
BEGIN
	...
	--card_pkg.SetGroupCards('Stub Invitation Landing', T_STRING_LIST('Chain.Cards.StubInvitationConfirmation', 'Chain.Cards.RejectRegisterLogin'));
	...
	card_pkg.SetGroupCards('Basic Company Filter', T_STRING_LIST('Chain.Cards.Filters.CompanyCore'));
END;

-- FROM \cvs\csr\db\utils\EnableChain\setup.sql
PROCEDURE Setup (
	in_chain_implementation		customer_options.chain_implementation%TYPE,
	in_initial_company_name		company.name%TYPE,
	in_link_pkg					customer_options.company_helper_sp%TYPE,
	in_site_name				VARCHAR2
)
AS
	...
	v_suppliers_group_sid		security_pkg.T_SID_ID;
...
BEGIN
	...
	BEGIN
		v_suppliers_group_sid := securableobject_pkg.GetSidFromPath(security_pkg.getACT, security_pkg.GetApp, 'Groups/Suppliers');
	EXCEPTION WHEN security_pkg.OBJECT_NOT_FOUND THEN
		security.group_pkg.CreateGroup(
			...
			'Suppliers',
			v_suppliers_group_sid
			);
	END;
	
	security.web_pkg.SetHomePage(security_pkg.getACT, security_pkg.GetApp, v_suppliers_group_sid, '/csr/site/chain/dashboard.acds');
	...
	v_host := securableobject_pkg.getname(security_pkg.getACT, security_pkg.getApp);
	...
	UPDATE customer_options
	   SET site_name = in_site_name, 
		   admin_has_dev_access = 0, -- Don't ever release a live site with this set to 1 (unless it's for demo purposes)
		   chain_implementation = (SELECT UPPER(in_chain_implementation) FROM DUAL), -- A way of applying changes to the same chain implementation that runs on different host names.
		   support_email = 'support@credit360.com',
		   company_helper_sp = in_link_pkg,
		   default_url = '/csr/site/chain/dashboard.acds'
	 WHERE app_sid = security_pkg.GetApp;
...
-- FROM \cvs\csr\db\chain\tools\enableChainPLSQL.sql
PROCEDURE CreateSOs (
	in_overwrite_default_url	NUMBER
)
AS
	v_act_id				security_pkg.T_ACT_ID DEFAULT security_pkg.GetAct;
	v_app_sid				security_pkg.T_SID_ID DEFAULT security_pkg.GetApp;
	...
	v_users_sid				security_pkg.T_SID_ID;
	--v_hidden_sid			security_pkg.T_SID_ID;
...
BEGIN
	...
	EXCEPTION
		WHEN security_pkg.OBJECT_NOT_FOUND then
			...
			acl_pkg.AddACE(v_act_id, Acl_pkg.GetDACLIDForSID(v_user_container_sid), security_pkg.ACL_INDEX_LAST, security_pkg.ACE_TYPE_ALLOW,
				security_pkg.ACE_FLAG_DEFAULT, v_users_sid, security_pkg.PERMISSION_STANDARD_ALL);	
	END;	
	...
	**************************************************************************************/
	...
	acl_pkg.DeleteAllAces(v_act_id, acl_pkg.GetDACLIDForSID(securableobject_pkg.GetSidFromPath(v_act_id, v_admin_menu, 'my_details')));
	...
	IF in_overwrite_default_url=1 THEN
...
AS
	v_site_name					customer_options.site_name%TYPE := CASE WHEN in_site_name IS NULL THEN in_initial_company_name||' Supplier' ELSE in_site_name END;
	v_chain_implementation		customer_options.chain_implementation%TYPE := CASE WHEN in_chain_implementation IS NULL THEN 'CSR.'||in_initial_company_name ELSE in_chain_implementation END;
BEGIN
	CreateSOs(in_overwrite_default_url);
	
	Setup(v_chain_implementation, in_initial_company_name, in_link_pkg, v_site_name);
	...
	CreateCompany(in_initial_company_name, in_initial_country_code);
	
	IF in_create_sectors = 1 THEN
		CreateSectors();
	END IF;
...
AS
	v_initial_company_name		company.name%TYPE DEFAULT 'Credit360';
	v_initial_country_code		company.country_code%TYPE DEFAULT 'gb';
	v_site_name					customer_options.site_name%TYPE;
	v_chain_implementation		customer_options.chain_implementation%TYPE;
	v_link_pkg					customer_options.company_helper_sp%TYPE;
BEGIN
	
	SELECT c.name, c.country_code, o.site_name, o.chain_implementation, o.company_helper_sp
	  INTO v_initial_company_name, v_initial_country_code, v_site_name, v_chain_implementation, v_link_pkg
	  FROM customer_options o
	  JOIN company c ON o.top_company_sid = c.company_sid;
	
	CreateSOs(0);
	
	Setup(v_chain_implementation, v_initial_company_name, v_link_pkg, v_site_name);
	...
	CreateCompany(v_initial_company_name, v_initial_country_code);
			...
			SELECT MIN(Task_Name) Task_Name, Task_Type_Id, MIN(position) position,
			  ...
			  (
				SELECT tt.description Task_Name, tt.Task_Type_Id, t.supplier_company_sid company_sid, tt.position,
					   CASE WHEN t.task_status_id IN (chain_pkg.TASK_OPEN) AND t.due_date between sysdate-7 AND sysdate		THEN my.column_value ELSE NULL END Due_Now,
					   CASE WHEN t.task_status_id IN (chain_pkg.TASK_OPEN) AND t.due_date between sysdate-14 AND sysdate-7	THEN my.column_value ELSE NULL END Over_Due,
					   CASE WHEN t.task_status_id IN (chain_pkg.TASK_OPEN) AND t.due_date < sysdate-14						THEN my.column_value ELSE NULL END Really_Over_Due,
					   CASE WHEN t.task_status_id IN (chain_pkg.TASK_OPEN) AND t.due_date between sysdate AND sysdate+7		THEN my.column_value ELSE NULL END Due_Soon
				  FROM task_type tt
				  ...
				  LEFT JOIN TABLE(v_my_companies) my ON t.supplier_company_sid=my.column_value
				 WHERE tt.app_sid = SYS_CONTEXT('SECURITY', 'APP')
				   AND tt.parent_task_type_id IS NULL
	...
	OPEN out_result_cur FOR 
		select sub.* from (
			SELECT sub_union.*, row_number() OVER (ORDER BY LOWER(sub_union.name)) rn, ctry.name as country_name
			  FROM (
				SELECT c.company_sid, c.name, c.country_code, c.active, 0 never_invited /*assumption here is any company that has been create properly has had someone invited before*/
				  FROM v$company c
				  JOIN TABLE(CAST(in_existing_results AS security.T_SID_TABLE)) T
					ON c.company_sid = T.column_value
				 WHERE c.app_sid = SYS_CONTEXT('SECURITY', 'APP')
				 UNION
				SELECT u.uninvited_supplier_sid, u.name, u.country_code, 0 active, NVL2(created_as_company_sid, 0, 1) never_invited /*assumption here is any company that has been create properly has had someone invited before*/
				  FROM uninvited_supplier u
...
BEGIN
	...
	(file_upload_sid, filename, mime_type, data, sha1, download_permission_id) 
...
BEGIN
	...
	CASE
		...
		WHEN ((v_permision = chain_pkg.DOWNLOAD_PERM_STND_TRANS) OR (v_permision = chain_pkg.DOWNLOAD_PERM_PRTCTD_TRANS)) THEN
			-- whether user can download this file is based on the transpancy settings for the application and
