...
ALTER TABLE csr.issue ADD description CLOB;
ALTER TABLE csr.issue ADD last_description CLOB;
ALTER TABLE csr.issue_action_log ADD old_description CLOB;
ALTER TABLE csr.issue_action_log ADD new_description CLOB;
ALTER TABLE csr.qs_expr_non_compl_action ADD non_comp_default_id NUMBER(10, 0);
...
create index csr.ix_issue_desc_search on csr.issue(description) indextype is ctxsys.context parameters('datastore ctxsys.default_datastore stoplist ctxsys.empty_stoplist');
...
ALTER TABLE csrimp.issue ADD description CLOB;
ALTER TABLE csrimp.issue ADD last_description CLOB;
ALTER TABLE csrimp.issue_action_log ADD old_description CLOB;
ALTER TABLE csrimp.issue_action_log ADD new_description CLOB;
ALTER TABLE csrimp.qs_expr_non_compl_action ADD non_comp_default_id NUMBER(10, 0);

BEGIN
	-- copy first log message into new description column, as per current behaviour
	FOR r IN (
		SELECT i.app_sid, i.issue_id, il.message
		  FROM csr.issue i
		  JOIN csr.issue_log il
		    ON i.first_issue_log_id = il.issue_log_id
	) LOOP
		UPDATE csr.issue
		   SET description = r.message
		 WHERE app_sid = r.app_sid
		   AND issue_id = r.issue_id;
	END LOOP;
	
	-- copy title into detail for existing survey non compliances, to preserve current behaviour
	UPDATE csr.qs_expr_non_compl_action
	   SET detail = title
