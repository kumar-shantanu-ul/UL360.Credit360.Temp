...
ALTER TABLE CSR.TEAMROOM_TYPE_TAB_GROUP ADD (
    IS_READ_ONLY        NUMBER(1, 0)     DEFAULT 1 NOT NULL,
    CHECK (IS_READ_ONLY IN (0,1))
);


ALTER TABLE CSR.INITIATIVE ADD (RAG_STATUS_ID NUMBER(10, 0));
...
ALTER TABLE CSR.INITIATIVE_PROJECT ADD (HELPER_PKG VARCHAR2(255));
...
CREATE OR REPLACE VIEW csr.v$issue AS
    SELECT i.app_sid, i.issue_id, i.label, i.description, i.source_label, i.is_visible, i.source_url, i.region_sid, re.description region_name, i.parent_id,
           ...
           sysdate now_dtm, due_dtm, forecast_dtm, ist.issue_type_Id, ist.label issue_type_label, ist.require_priority, ist.allow_children, ist.can_be_public, ist.show_forecast_dtm, i.issue_priority_id, ip.due_date_offset, ip.description priority_description, ist.require_var_expl,
